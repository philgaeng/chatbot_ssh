#!/usr/bin/env python3
"""
Server initialization script for Nepal Chatbot.

This script provides a robust server management system for the Nepal Chatbot application.
It handles the initialization, monitoring, and graceful shutdown of all required services.

Features:
- Process Management:
  * Runs each server in its own subprocess for better isolation
  * Monitors process health and handles unexpected terminations
  * Provides graceful shutdown on system signals (SIGINT, SIGTERM)

- Server Components:
  1. Database initialization and management
  2. File server (port 5001) for handling uploads
  3. Accessibility web app (port 5006)
  4. Rasa action server (port 5055)
  5. Main Rasa server (port 5005)

- Logging System:
  * Separate log files for each component
  * Configurable log retention period
  * Rotating file handlers with daily rotation
  * Console and file output

- Port Management:
  * Automatic port availability checking
  * Cleanup of existing processes on required ports
  * Configurable ports via command-line arguments

Usage:
    python run_servers.py [options]

Options:
    --all                    Run all servers
    --file-server-only       Run only the file server
    --accessibility-only     Run only the accessibility app
    --rasa-action-only       Run only the Rasa action server
    --rasa-server-only       Run only the main Rasa server
    --skip-db-init          Skip database initialization
    --log-retention DAYS    Set log retention period in days (default: 90)
    --config FILE           Path to configuration file (default: config.json)

Port Configuration:
    --file-server-port PORT     File server port (default: 5001)
    --accessibility-port PORT   Accessibility app port (default: 5006)
    --rasa-action-port PORT     Rasa action server port (default: 5055)
    --rasa-server-port PORT     Main Rasa server port (default: 5005)

Best Practices:
1. Always use --all for production deployment
2. Monitor logs in logs/ directory for each component
3. Use --skip-db-init only when database is already initialized
4. Configure appropriate log retention based on storage capacity
"""

import os
import sys
import logging
import threading
import time
import argparse
import subprocess
import signal
import psutil
import socket
import json
from dataclasses import dataclass, asdict
from typing import Optional, Dict, Any, List
from logging.handlers import TimedRotatingFileHandler

# Add the project root directory to the Python path
project_root = os.path.dirname(os.path.abspath(__file__))
sys.path.append(project_root)

@dataclass
class ServerConfig:
    """Server configuration class"""
    # Server ports
    file_server_port: int = 5001
    accessibility_port: int = 5006
    rasa_action_port: int = 5055
    rasa_server_port: int = 5005
    
    # Process management
    process_check_interval: int = 30
    process_restart_attempts: int = 3
    process_restart_delay: int = 5
    
    # Resource limits
    max_memory_percent: int = 80
    max_cpu_percent: int = 90
    max_disk_percent: int = 90
    
    # Logging
    log_dir: str = "logs"
    log_max_size_mb: int = 100
    log_max_files: int = 5
    log_format: str = "json"  # or "text"
    log_retention_days: int = 90
    
    # Health check
    health_check_timeout: int = 30
    health_check_retries: int = 3
    health_check_delay: int = 5
    
    # Error patterns to monitor
    error_patterns: list = None
    
    def __post_init__(self):
        if self.error_patterns is None:
            self.error_patterns = [
                "connection refused",
                "address already in use",
                "permission denied",
                "out of memory",
                "timeout",
                "failed to start",
                "process terminated",
                "port already in use"
            ]
    
    @classmethod
    def from_file(cls, filepath: str) -> 'ServerConfig':
        """Load configuration from a JSON file"""
        try:
            with open(filepath, 'r') as f:
                config_dict = json.load(f)
            return cls(**config_dict)
        except Exception as e:
            print(f"Error loading config file: {e}")
            return cls()
    
    @classmethod
    def from_env(cls) -> 'ServerConfig':
        """Load configuration from environment variables"""
        config = cls()
        for field in config.__dataclass_fields__:
            env_value = os.getenv(field.upper())
            if env_value is not None:
                # Convert string values to appropriate types
                field_type = type(getattr(config, field))
                if field_type == int:
                    setattr(config, field, int(env_value))
                elif field_type == float:
                    setattr(config, field, float(env_value))
                elif field_type == bool:
                    setattr(config, field, env_value.lower() in ('true', '1', 'yes'))
                else:
                    setattr(config, field, env_value)
        return config
    
    def to_file(self, filepath: str) -> None:
        """Save configuration to a JSON file"""
        with open(filepath, 'w') as f:
            json.dump(asdict(self), f, indent=4)
    
    def get_ports(self) -> List[int]:
        """Get list of all configured ports"""
        return [
            self.file_server_port,
            self.accessibility_port,
            self.rasa_action_port,
            self.rasa_server_port
        ]
    
    def validate(self) -> List[str]:
        """Validate configuration values"""
        errors = []
        
        # Validate ports
        for port in self.get_ports():
            if not (1024 <= port <= 65535):
                errors.append(f"Port {port} must be between 1024 and 65535")
        
        # Validate percentages
        for percent in [self.max_memory_percent, self.max_cpu_percent, self.max_disk_percent]:
            if not (0 <= percent <= 100):
                errors.append(f"Percentage {percent} must be between 0 and 100")
        
        # Validate timeouts and delays
        if self.health_check_timeout < 1:
            errors.append("Health check timeout must be at least 1 second")
        if self.health_check_delay < 1:
            errors.append("Health check delay must be at least 1 second")
        
        # Validate log settings
        if self.log_retention_days < 1:
            errors.append("Log retention days must be at least 1")
        if self.log_max_files < 1:
            errors.append("Log max files must be at least 1")
        if self.log_max_size_mb < 1:
            errors.append("Log max size must be at least 1 MB")
        
        return errors

def setup_logging(config: ServerConfig) -> tuple:
    """Set up logging with specified retention period"""
    # Create logs directory if it doesn't exist
    logs_dir = os.path.join(project_root, config.log_dir)
    if not os.path.exists(logs_dir):
        os.makedirs(logs_dir)
        print(f"Created logs directory: {logs_dir}")
    
    # Common log format for all loggers
    log_format = '%(asctime)s - %(levelname)s - [%(name)s] - %(message)s'
    log_formatter = logging.Formatter(log_format)
    
    # Root logger configuration (console output)
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.INFO)
    
    # Console handler for root logger
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(log_formatter)
    root_logger.addHandler(console_handler)
    
    # Create separate logger for main script
    server_manager_logger = logging.getLogger('ServerManager')
    # Create file handler for main script
    main_log_file = os.path.join(logs_dir, 'server_manager.log')
    main_file_handler = TimedRotatingFileHandler(
        main_log_file,
        when='midnight',
        interval=1,
        backupCount=config.log_retention_days
    )
    main_file_handler.setFormatter(log_formatter)
    server_manager_logger.addHandler(main_file_handler)
    
    # Create loggers for each component
    rasa_server_logger = logging.getLogger('RasaServer')
    rasa_action_logger = logging.getLogger('RasaActionServer')
    file_server_logger = logging.getLogger('FileServer')
    accessible_logger = logging.getLogger('AccessibleServer')
    
    # Set up file handlers for each component
    rasa_server_log_file = os.path.join(logs_dir, 'rasa_server.log')
    rasa_action_log_file = os.path.join(logs_dir, 'actions_server.log')
    file_server_log_file = os.path.join(logs_dir, 'file_server.log')
    accessible_log_file = os.path.join(logs_dir, 'accessible_server.log')
    
    # Rasa server file handler
    rasa_server_file_handler = TimedRotatingFileHandler(
        rasa_server_log_file,
        when='midnight',
        interval=1,
        backupCount=config.log_retention_days
    )
    rasa_server_file_handler.setFormatter(log_formatter)
    rasa_server_logger.addHandler(rasa_server_file_handler)
    
    # Rasa action server file handler
    rasa_action_file_handler = TimedRotatingFileHandler(
        rasa_action_log_file,
        when='midnight',
        interval=1,
        backupCount=config.log_retention_days
    )
    rasa_action_file_handler.setFormatter(log_formatter)
    rasa_action_logger.addHandler(rasa_action_file_handler)
    
    # File server file handler
    file_server_file_handler = TimedRotatingFileHandler(
        file_server_log_file,
        when='midnight',
        interval=1,
        backupCount=config.log_retention_days
    )
    file_server_file_handler.setFormatter(log_formatter)
    file_server_logger.addHandler(file_server_file_handler)
    
    # Accessible server file handler
    accessible_file_handler = TimedRotatingFileHandler(
        accessible_log_file,
        when='midnight',
        interval=1,
        backupCount=config.log_retention_days
    )
    accessible_file_handler.setFormatter(log_formatter)
    accessible_logger.addHandler(accessible_file_handler)
    
    print(f"Logging initialized with {config.log_retention_days} days retention")
    return (
        server_manager_logger,
        rasa_server_logger,
        rasa_action_logger,
        file_server_logger,
        accessible_logger,
        file_server_file_handler,
        accessible_file_handler
    )

# Initialize global logger variables
logger = None
rasa_server_logger = None
rasa_action_logger = None
file_server_logger = None
accessible_logger = None
file_server_file_handler = None
accessible_file_handler = None

def init_database():
    """Initialize the database"""
    logger.info("Initializing database...")
    
    try:
        # Initialize the database using the shared db_manager
        from backend.services.database_services.postgres_services import db_manager
        db_manager.init_db()
        logger.info("Database initialized successfully")
        return True
    except Exception as e:
        logger.error(f"Database initialization error: {str(e)}", exc_info=True)
        return False

def start_file_server(port=5001):
    """Start the file server for handling uploads"""
    file_server_logger.info(f"Starting file server on port {port}...")
    
    try:
        # First, ensure actions_server is imported and initialized
        from actions_server import db_manager
        
        # Import file server app
        from backend.api.app import app as file_server_app
        
        # Create uploads directory if it doesn't exist
        uploads_dir = os.path.join(project_root, 'uploads')
        if not os.path.exists(uploads_dir):
            os.makedirs(uploads_dir)
            file_server_logger.info(f"Created uploads directory: {uploads_dir}")
        
        # Set up logging for Flask app
        import logging as flask_logging
        flask_app_logger = flask_logging.getLogger('werkzeug')
        flask_app_logger.setLevel(flask_logging.INFO)
        flask_app_logger.addHandler(file_server_file_handler)
        
        # Start the file server in a separate process
        cmd = [
            sys.executable,
            "-c",
            f"""
import sys
from backend.api.app import app
app.run(host='0.0.0.0', port={port}, use_reloader=False, debug=False)
            """
        ]
        
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True
        )
        
        # Log output in a separate thread
        def log_output():
            for line in process.stdout:
                file_server_logger.info(line.strip())
            for line in process.stderr:
                file_server_logger.error(line.strip())
                
        threading.Thread(target=log_output, daemon=True).start()
        
        # Give the server a moment to start
        time.sleep(2)
        if process.poll() is not None:
            file_server_logger.error(f"File server failed to start, exit code: {process.poll()}")
            return None
            
        file_server_logger.info(f"File server started on port {port}")
        return process
    except Exception as e:
        file_server_logger.error(f"Error starting file server: {str(e)}", exc_info=True)
        return None

def start_accessible_app(port=5006):
    """Start the accessibility web app"""
    accessible_logger.info(f"Starting accessibility app on port {port}...")
    
    try:
        # Start the accessibility app in a separate process
        cmd = [
            sys.executable,
            "-c",
            f"""
import os
import sys

# Add the project root directory to the Python path
project_root = '{project_root}'
sys.path.append(project_root)

# Now import the app
from backend.api.app import app

# Add a health check endpoint
@app.route('/health')
def health_check():
    return 'OK', 200

# Add language-specific routes to match nginx configuration
@app.route('/accessible/en/')
def serve_english():
    return app.send_static_file('index_en.html')

@app.route('/accessible/ne/')
def serve_nepali():
    return app.send_static_file('index.html')

@app.route('/accessible/')
def redirect_to_language():
    # Default to Nepali version
    return app.send_static_file('index.html')

# Configure static files
app.static_folder = os.path.join(project_root, 'channels', 'accessible')
app.static_url_path = '/accessible'

app.run(host='0.0.0.0', port={port}, use_reloader=False, debug=False)
            """
        ]
        
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True,
            cwd=project_root  # Set working directory to project root
        )
        
        # Log output in a separate thread
        def log_output():
            for line in process.stdout:
                accessible_logger.info(line.strip())
            for line in process.stderr:
                accessible_logger.error(line.strip())
                
        threading.Thread(target=log_output, daemon=True).start()
        
        # Give the server a moment to start
        time.sleep(2)
        if process.poll() is not None:
            accessible_logger.error(f"Accessibility app failed to start, exit code: {process.poll()}")
            return None
            
        accessible_logger.info(f"Accessibility app started on port {port}")
        return process
    except Exception as e:
        accessible_logger.error(f"Error starting accessibility app: {str(e)}", exc_info=True)
        return None

def start_rasa_action_server(port=5055):
    """Start the Rasa action server"""
    rasa_action_logger.info(f"Starting Rasa action server on port {port}...")
    
    try:
        import subprocess
        
        # Check if port is available before starting
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            sock.bind(('0.0.0.0', port))
            sock.close()
        except OSError as e:
            rasa_action_logger.error(f"Port {port} is not available for Rasa action server: {e}")
            # Try to find what's using the port
            try:
                result = subprocess.run(["lsof", "-i", f":{port}"], capture_output=True, text=True)
                if result.stdout:
                    rasa_action_logger.error(f"Port {port} is in use by:\n{result.stdout}")
                    # Try one last kill
                    subprocess.run(["fuser", "-k", f"{port}/tcp"], capture_output=True)
                    time.sleep(2)
            except Exception as e:
                rasa_action_logger.error(f"Error checking port {port}: {e}")
            return None
        
        # Start the Rasa action server with the specified port
        cmd = [
            "rasa", "run", "actions", 
            "--port", str(port),
            "--actions", "actions"
        ]
        
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True
        )
        
        # Log output in a separate thread
        def log_output():
            for line in process.stdout:
                rasa_action_logger.info(line.strip())
            for line in process.stderr:
                rasa_action_logger.error(line.strip())
                
        threading.Thread(target=log_output, daemon=True).start()
        
        # Give Rasa a moment to start and check if it's running
        time.sleep(5)
        if process.poll() is not None:
            rasa_action_logger.error(f"Rasa action server failed to start, exit code: {process.poll()}")
            return None
            
        rasa_action_logger.info(f"Rasa action server started on port {port}")
        return process
    except Exception as e:
        rasa_action_logger.error(f"Error starting Rasa action server: {str(e)}", exc_info=True)
        return None

def start_rasa_server(port=5005):
    """Start the main Rasa server"""
    rasa_server_logger.info(f"Starting Rasa server on port {port}...")
    
    try:
        import subprocess
        
        # Check if port is available before starting
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            sock.bind(('0.0.0.0', port))
            sock.close()
        except OSError as e:
            rasa_server_logger.error(f"Port {port} is not available for Rasa server: {e}")
            # Try to find what's using the port
            try:
                result = subprocess.run(["lsof", "-i", f":{port}"], capture_output=True, text=True)
                if result.stdout:
                    rasa_server_logger.error(f"Port {port} is in use by:\n{result.stdout}")
                    # Try one last kill
                    subprocess.run(["fuser", "-k", f"{port}/tcp"], capture_output=True)
                    time.sleep(2)
            except Exception as e:
                rasa_server_logger.error(f"Error checking port {port}: {e}")
            return None
        
        # Start the main Rasa server
        cmd = [
            "rasa", "run", 
            "--port", str(port),
            "--enable-api",
            "--cors", "*"
        ]
        
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True
        )
        
        # Log output in a separate thread
        def log_output():
            for line in process.stdout:
                rasa_server_logger.info(line.strip())
            for line in process.stderr:
                rasa_server_logger.error(line.strip())
                
        threading.Thread(target=log_output, daemon=True).start()
        
        # Give Rasa a moment to start and check if it's running
        time.sleep(5)
        if process.poll() is not None:
            rasa_server_logger.error(f"Rasa server failed to start, exit code: {process.poll()}")
            return None
            
        rasa_server_logger.info(f"Rasa server started on port {port}")
        return process
    except Exception as e:
        rasa_server_logger.error(f"Error starting Rasa server: {str(e)}", exc_info=True)
        return None

def check_server(url, name, max_attempts=5):
    """Check if a server is running by making an HTTP request"""
    import requests
    from requests.exceptions import RequestException
    
    for attempt in range(max_attempts):
        try:
            # Add a timeout and verify=False for development environments
            response = requests.get(url, timeout=2, verify=False)
            if response.status_code == 200:
                logger.info(f"{name} is running at {url}")
                return True
            else:
                logger.warning(f"{name} returned status code {response.status_code} on attempt {attempt+1}")
        except RequestException as e:
            logger.warning(f"Failed to connect to {name} on attempt {attempt+1}: {str(e)}")
        
        if attempt < max_attempts - 1:
            logger.info(f"Waiting for {name} to start (attempt {attempt+1}/{max_attempts})...")
            time.sleep(2)
    
    logger.error(f"{name} failed to start after {max_attempts} attempts")
    return False

def stop_servers_on_ports(ports):
    """Stop any processes listening on the specified ports
    
    Args:
        ports (list): List of ports to check and stop processes on
    """
    logger.info(f"Checking for existing processes on ports: {ports}")
    
    for port in ports:
        try:
            # Using netstat to find processes bound to the specified port
            result = subprocess.run(
                ["lsof", "-i", f":{port}"], 
                capture_output=True, 
                text=True
            )
            
            if result.stdout:
                # Extract PIDs from the output
                lines = result.stdout.strip().split('\n')
                if len(lines) > 1:  # Skip header line
                    for line in lines[1:]:
                        parts = line.split()
                        if len(parts) > 1:
                            pid = int(parts[1])
                            try:
                                process = psutil.Process(pid)
                                process_name = process.name()
                                logger.info(f"Stopping process {process_name} (PID: {pid}) on port {port}")
                                os.kill(pid, signal.SIGTERM)
                                time.sleep(1)  # Give it a moment to shut down
                                
                                # Check if it's still running and force kill if necessary
                                if psutil.pid_exists(pid):
                                    logger.info(f"Process {pid} still running, forcing termination")
                                    os.kill(pid, signal.SIGKILL)
                                    
                                    # Wait and check again
                                    time.sleep(2)
                                    if psutil.pid_exists(pid):
                                        logger.warning(f"Process {pid} could not be terminated, trying another approach")
                                        
                                        # Try more aggressive approach if process still exists
                                        try:
                                            subprocess.run(["kill", "-9", str(pid)], check=True)
                                            time.sleep(1)
                                        except subprocess.CalledProcessError:
                                            logger.error(f"Failed to kill process {pid} with kill -9")
                            except (psutil.NoSuchProcess, ProcessLookupError):
                                logger.info(f"Process {pid} already terminated")
                            except Exception as e:
                                logger.error(f"Error stopping process {pid}: {e}")
                                
            # After attempting to kill processes, verify the port is actually free
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
                sock.bind(('0.0.0.0', port))
                sock.close()
                logger.info(f"Port {port} is now available")
            except OSError:
                logger.error(f"Port {port} is still in use after kill attempts!")
                
                # Last resort - try finding any process still using the port
                result = subprocess.run(
                    ["fuser", "-k", f"{port}/tcp"], 
                    capture_output=True, 
                    text=True
                )
                logger.info(f"Attempted fuser kill on port {port}: {result.stdout}")
                time.sleep(2)
        except Exception as e:
            logger.error(f"Error checking for processes on port {port}: {e}")

def stop_all_rasa_processes():
    """Kill all Rasa-related processes"""
    logger.info("Stopping all Rasa processes...")
    try:
        # Find all Rasa processes
        subprocess.run(["pkill", "-f", "rasa"], capture_output=True)
        time.sleep(2)
        # Force kill any remaining Rasa processes
        subprocess.run(["pkill", "-9", "-f", "rasa"], capture_output=True)
    except Exception as e:
        logger.error(f"Error stopping Rasa processes: {e}")

def main():
    """Main function to run servers"""
    parser = argparse.ArgumentParser(description='Start Nepal Chatbot servers')
    parser.add_argument('--file-server-port', type=int, help='Port for file server')
    parser.add_argument('--accessibility-port', type=int, help='Port for accessibility app')
    parser.add_argument('--rasa-action-port', type=int, help='Port for Rasa action server')
    parser.add_argument('--rasa-server-port', type=int, help='Port for main Rasa server')
    parser.add_argument('--skip-db-init', action='store_true', help='Skip database initialization')
    parser.add_argument('--file-server-only', action='store_true', help='Run only the file server')
    parser.add_argument('--accessibility-only', action='store_true', help='Run only the accessibility app')
    parser.add_argument('--rasa-action-only', action='store_true', help='Run only the Rasa action server')
    parser.add_argument('--rasa-server-only', action='store_true', help='Run only the main Rasa server')
    parser.add_argument('--all', action='store_true', help='Run all servers')
    parser.add_argument('--log-retention', type=int, help='Log retention period in days')
    parser.add_argument('--config', type=str, help='Path to configuration file')
    
    args = parser.parse_args()
    
    # Load configuration
    config = ServerConfig.from_env()  # First load from environment
    
    if args.config and os.path.exists(args.config):
        config = ServerConfig.from_file(args.config)  # Then override with config file
    
    # Override with command line arguments
    for arg in vars(args):
        if getattr(args, arg) is not None:
            setattr(config, arg, getattr(args, arg))
    
    # Validate configuration
    errors = config.validate()
    if errors:
        print("Configuration errors:")
        for error in errors:
            print(f"  - {error}")
        sys.exit(1)
    
    # Initialize logging with specified retention
    global logger, rasa_server_logger, rasa_action_logger, file_server_logger, accessible_logger
    global file_server_file_handler, accessible_file_handler
    
    logger, rasa_server_logger, rasa_action_logger, file_server_logger, accessible_logger, \
    file_server_file_handler, accessible_file_handler = setup_logging(config)
    
    # If no specific server is selected, show help
    if not (args.file_server_only or args.accessibility_only or args.rasa_action_only or args.rasa_server_only or args.all):
        parser.print_help()
        return
    
    # Stop all Rasa processes first to ensure clean start
    stop_all_rasa_processes()
    
    # Build list of ports that will be used
    ports_to_check = []
    if args.file_server_only or args.all:
        ports_to_check.append(config.file_server_port)
    if args.accessibility_only or args.all:
        ports_to_check.append(config.accessibility_port)
    if args.rasa_action_only or args.all:
        ports_to_check.append(config.rasa_action_port)
    if args.rasa_server_only or args.all:
        ports_to_check.append(config.rasa_server_port)
    
    # Stop any existing servers on these ports
    stop_servers_on_ports(ports_to_check)
    
    # Double-check that all ports are available before proceeding
    unavailable_ports = []
    for port in ports_to_check:
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            sock.bind(('0.0.0.0', port))
            sock.close()
        except OSError:
            unavailable_ports.append(port)
            logger.error(f"Port {port} is still not available after cleanup attempts")
    
    if unavailable_ports:
        logger.error(f"Ports {unavailable_ports} are still in use. Please stop services manually or try again later.")
        return
    
    # Initialize database unless skipped
    if not args.skip_db_init:
        if not init_database():
            logger.error("Database initialization failed, exiting")
            return
    
    processes = []
    
    # Start file server if requested
    file_server_process = None
    if args.file_server_only or args.all:
        file_server_process = start_file_server(config.file_server_port)
        if file_server_process:
            processes.append(file_server_process)
            # Give the file server a moment to start
            time.sleep(2)
            check_server(f"http://localhost:{config.file_server_port}", "File server")
    
    # Start Rasa action server if requested
    rasa_action_process = None
    if args.rasa_action_only or args.all:
        rasa_action_process = start_rasa_action_server(config.rasa_action_port)
        if rasa_action_process:
            processes.append(rasa_action_process)
            # Give the Rasa action server a moment to start
            time.sleep(2)
    
    # Start main Rasa server if requested
    rasa_server_process = None
    if args.rasa_server_only or args.all:
        rasa_server_process = start_rasa_server(config.rasa_server_port)
        if rasa_server_process:
            processes.append(rasa_server_process)
            # Give the Rasa server a moment to start
            time.sleep(2)
    
    # Start accessibility app if requested
    accessibility_process = None
    if args.accessibility_only or args.all:
        accessibility_process = start_accessible_app(config.accessibility_port)
        if accessibility_process:
            processes.append(accessibility_process)
            # Give the accessibility app a moment to start
            time.sleep(2)
            check_server(f"http://localhost:{config.accessibility_port}/health", "Accessibility app")
    
    # Set up signal handlers for graceful shutdown
    def signal_handler(signum, frame):
        logger.info(f"Received signal {signum}, shutting down gracefully...")
        for process in processes:
            if process and process.poll() is None:
                process.terminate()
        sys.exit(0)
    
    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)
    
    # Keep the main thread alive as long as any server is running
    try:
        while True:
            for process in processes:
                if process and process.poll() is not None:
                    logger.error(f"Process {process.pid} has stopped unexpectedly")
                    # Terminate all other processes
                    for p in processes:
                        if p and p.poll() is None:
                            p.terminate()
                    return
            time.sleep(1)
    except KeyboardInterrupt:
        logger.info("Received keyboard interrupt, shutting down")
        # Terminate any subprocess
        for process in processes:
            if process and process.poll() is None:
                process.terminate()
    
if __name__ == "__main__":
    main() 