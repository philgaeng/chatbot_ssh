...
def test_standard_org_admin_can_manage_structure():
    user = CurrentUser(
        user_id="c@grm.local",
        role_keys=["org_admin"],
    ...
    assert is_org_admin(user, "standard")
    assert not is_org_admin(user, "seah")


def test_seah_org_admin_can_manage_structure():
    user = CurrentUser(
    ...
    assert can_manage_structure(user)
    assert is_org_admin(user, "seah")


def test_org_admin_can_create_project_either_track():
...
def test_require_structure_ok_for_seah_org_admin():
    user = CurrentUser(
        user_id="seah@grm.local",
        role_keys=["org_admin"],
        admin_scopes=[_scope(workflow_track="seah")],
...
def test_org_admin_dual_track_scopes():
    user = CurrentUser(
