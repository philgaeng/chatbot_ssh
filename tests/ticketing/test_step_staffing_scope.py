...
@contextmanager
def _l1_scope(db, project, per_lot: bool):
    ...
    standing fact. It is not a fact, it is a checkbox: an admin ticked **Staffed for each lot**
    ...
    try:
        step.staff_per_package = per_lot
        db.flush()
        yield step
    finally:
...
def test_a_project_wide_level_passes_on_one_officer(db, kl_road_project):
    """KL Road has lots with no per-lot officer. With Level 1 declared project-wide, its one
    project-wide officer is a complete answer."""
    with _l1_scope(db, kl_road_project, per_lot=False):
...
def test_a_per_lot_level_is_not_satisfied_by_a_project_wide_officer(db, kl_road_project):
    ...
    with _l1_scope(db, kl_road_project, per_lot=True):
