...
/** The Web Lock every tab of this origin takes before renewing (`GRM-105`). */
export const RENEWAL_LOCK = "grm-token-renewal";

/** A renewal that hangs must not hold the lock — and every other tab's API calls — forever. */
const RENEWAL_TIMEOUT_MS = 15_000;

/**
 * Run `fn` holding the cross-tab renewal lock.
 *
 * ⚠ **Why a lock across tabs, not only single-flight within one.** The realm revokes a refresh
 * token once it has been used, and — measured on Keycloak — a SECOND use ends the whole session,
 * the new token included. Two open tabs renewing in the same instant would therefore sign the
 * officer out of every tab. `refreshPromise` dedupes callers inside a tab; this dedupes tabs.
 *
 * ⚠ **Without Web Locks** (browsers older than 2022: Safari < 15.4, Firefox < 96) this runs
 * unlocked. The residual is the race above, which needs two tabs renewing within one request's
 * round trip; it is stated in `16_auth_keycloak.md` rather than papered over with a
 * `localStorage` lease, which is itself racy.
 */
function withRenewalLock<T>(fn: () => Promise<T>): Promise<T> {
  const locks = typeof navigator !== "undefined" ? navigator.locks : undefined;
  if (!locks?.request) return fn();
  return locks.request(RENEWAL_LOCK, { mode: "exclusive" }, fn) as Promise<T>;
}
 ...
 *
 * **Across tabs, too (`GRM-105`):** see `withRenewalLock`. Since the realm revokes spent refresh
 * tokens, a second use is no longer a harmless 400 — it ends the session everywhere.
 */
...
async function doRefresh(): Promise<string | null> {
  ...
  //
  // The token this tab set out to renew — read BEFORE waiting for the lock, so that after the wait
  // we can tell whether another tab renewed in the meantime.
  const seen = localStorage.getItem(TOKEN_STORAGE.REFRESH_TOKEN);
  if (!seen) return null;

  return withRenewalLock(async () => {
    const refreshToken = localStorage.getItem(TOKEN_STORAGE.REFRESH_TOKEN);
    if (!refreshToken) return null; // signed out while we waited

    // ⭐ Another tab renewed while we waited. Its tokens are already in storage, and posting the
    // token we started with would be a reuse — which ends the session for every tab. Use theirs.
    if (refreshToken !== seen) {
      const access = localStorage.getItem(TOKEN_STORAGE.ACCESS_TOKEN);
      if (access && !isAccessTokenExpiringSoon(access, 60)) return access;
    }
    ...
    try {
      ...
      return null; // aborted, offline, or unparseable
    } finally {
      clearTimeout(timer);
    }
  });
