...
test("acknowledge → attach a photo → resolve", async ({ page, asOfficer }, testInfo) => {
  ...
  // GRM-117: "I did" is the default and names the officer's office, never the officer.
  await expect(page.getByRole("radio", { name: /^I did/ })).toBeChecked();
  ...
  await expect
    ...
    .toBe("RESOLVED");
  await expect(page.getByText(/Resolved by: /).first()).toBeVisible();
...
/**
 * GRM-117: the two other answers to *Who took the action?* — each resolves a fresh ticket, and the
 * thread's resolution record names who acted. ADB is chosen from "Offices on this project" (KL Road
 * names it as donor on every seed), the police from the fixed list: nothing is typed.
 */
for (const variant of [
  { kind: "Another office", label: "Asian Development Bank (ADB)" },
  { kind: "An outside body", label: "Police" },
] as const) {
  test(`resolve — who took the action: ${variant.kind}`, async ({ page, asOfficer }, testInfo) => {
    const ticket = await createTicket(`resolve-actor-${variant.kind}`);
    await asOfficer(asAssignee(ticket.assignee));
    ...
    await page.getByRole("radio", { name: variant.kind }).check();
    // Picking the kind alone is not an answer: Confirm waits for the office or body.
    await expect(confirm).toBeDisabled();
    if (variant.kind === "Another office") {
      await page.getByRole("radio", { name: variant.label }).check();
    } else {
      await page.getByLabel("Which outside body").selectOption({ label: variant.label });
    }
    await captureScreenshot(page, testInfo, `flow-resolve-actor-${variant.kind === "Another office" ? "office" : "outside"}`);
    await confirm.click();

    await expect
      .poll(async () => (await getTicket(ticket.ticketId)).status_code, { timeout: 20_000 })
      .toBe("RESOLVED");
    await expect(page.getByText(`Resolved by: ${variant.label}`).first()).toBeVisible();
  });
}
...
test("a SEAH case resolves with the text alone — no action is recorded", async ({ page, asOfficer }, testInfo) => {
  ...
  await expect(page.getByText("Who took the action?")).toHaveCount(0);  // GRM-117: nor who did it
