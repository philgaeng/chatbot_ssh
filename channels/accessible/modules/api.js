...
export default function createAPIModule() {
  return {
    ...
    init: function () {
      ...
      this.baseUrl =
        typeof APP_CONFIG !== "undefined" && APP_CONFIG.api?.baseUrl
          ? APP_CONFIG.api.baseUrl
          : "";
      this.endpoints =
        typeof APP_CONFIG !== "undefined" && APP_CONFIG.api?.endpoints
          ? APP_CONFIG.api.endpoints
          : {};
     ...
     * @param {FormData} [additionalFormData] - Optional FormData with additional fields like grievance_id, complainant_id, language_code
     * @returns {Promise}
     */
    submitGrievance: async function (userInfo, recordings, additionalFormData) {
      ...
      // Add additional form data if provided (grievance_id, complainant_id, language_code, etc.)
      ...
      for (const [key, value] of formData.entries()) {
        if (value instanceof File || value instanceof Blob) {
          console.log(
            `- ${key}: ${value.constructor.name} (${value.size} bytes)`
          );
        } else {
    ...
    /**
     * Generates grievance_id and complainant_id using centralized logic
