...
export default function createReviewDataModule(socket, UIModule, RecordingModule, GrievanceModule) {
    return {
        ...
        init: function() {
            ...
            socket.on('status_update:grievance', this.handleTaskCompletion.bind(this));
            ...
            console.log('[ReviewData] Module initialized - access via window.debugReviewData');
        },
        ...
        setupTextEditing: function() {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (e.target.textContent.includes('Modify Categories') || e.target.classList.contains('modify-categories-btn')) {
        ...
        getEditAreaId: function(fieldName) {
            const mapping = {
                'grievance_details': 'grievanceDetailsEditArea',
                'grievance_summary': 'grievanceSummaryEditArea',
                'user_full_name': 'userNameEditArea',
                'user_contact_phone': 'userPhoneEditArea',
                'user_municipality': 'userMunicipalityEditArea',
                'user_village': 'userVillageEditArea',
                'user_address': 'userAddressEditArea'
            };
        ...
        getTextareaId: function(fieldName) {
            const mapping = {
                'grievance_details': 'grievanceDetailsEdit',
                'grievance_summary': 'grievanceSummaryEdit',
                'user_full_name': 'userNameEdit',
                'user_contact_phone': 'userPhoneEdit',
                'user_municipality': 'userMunicipalityEdit',
                'user_village': 'userVillageEdit',
                'user_address': 'userAddressEdit'
            };
            return mapping[fieldName];
        },
        ...
        storeTaskResult: function(result) {
            ...
            if (result.values && typeof result.values === 'object') {
                Object.entries(result.values).forEach(([key, value]) => {
                    if (key !== 'field_name' && this.fieldMapping[key]) {
                        extractedData[key] = value;
                        console.log(`[ReviewData] Extracted from values: ${key} = ${value}`);
                    }
                });
            }
            ...
            if (result.value && typeof result.value === 'object') {
                Object.entries(result.value).forEach(([key, value]) => {
                    if (key !== 'field_name' && this.fieldMapping[key]) {
                        extractedData[key] = value;
                        console.log(`[ReviewData] Extracted from nested value: ${key} = ${value}`);
                    }
                });
                
                // Handle categories specially (they come as an array)
                if (result.value.grievance_categories) {
                    extractedData.grievance_categories = result.value.grievance_categories;
                }
        ...
        updateReviewUI: function(result) {
            console.log('[ReviewData] Updating UI with stored data:', this.reviewData);
                ...
                if (elementId) {
                    ...
                    if (element) {
                        ...
                        if (!this.editedData[fieldName]) {
                            ...
                            console.log(`[ReviewData] Skipped updating ${elementId} - user has edited this field`);
                        }
                    }
                }
            });
        ...
        createCategoryDropdown: function(categories) {
            ...
            const categoryElement = document.getElementById('grievanceCategoriesReview');
            if (!categoryElement) {
            ...
            const currentCategories = this.editedData.grievance_categories || this.reviewData.grievance_categories || [];
            const currentCategory = Array.isArray(currentCategories) ? currentCategories[0] : currentCategories;
            
            // Create a container for the dropdown
            let dropdownContainer = document.getElementById('categoryDropdownContainer');
            if (!dropdownContainer) {
                ...
                categoryElement.parentNode.insertBefore(dropdownContainer, categoryElement.nextSibling);
            }
            ...
            console.log('[ReviewData] Created category dropdown with', categories.length, 'categories');
        },
        ...
        updateCategoryDisplay: function(categories) {
            ...
            if (categoryElement) {
                ...
                categoryElement.innerHTML = categories.map(cat => 
