...
from typing import Dict, Text, Any, Tuple, List, Callable, Optional, Literal
...
class ActionFlowHelpersMixin(ActionHelpersMixin):
    ...
    def _retrieve_and_set_grievances_by_phone(
        ...
        self.logger.info(
            "%s: _retrieve_and_set_grievances_by_phone entry | phone=%s",
            self.name(),
            complainant_phone,
        )
        ...
        if not complainant_phone or complainant_phone == self.DEFAULT_VALUES['SKIP_VALUE']:
            self.logger.info("%s: Phone skipped or not provided", self.name())
            return {
        ...
        try:
            list_grievances_by_phone = self.db_manager.get_grievance_by_complainant_phone(
                complainant_phone
            )
        except Exception as e:
    ...
    def get_complainant_slot_events_from_grievance(
        ...
        """
        Load complainant data for a given grievance_id and return SlotSet events
        for commonly used complainant fields.

        This centralizes the "hydrate tracker from DB" logic so it can be reused
        by any flow (status check, modify grievance, etc.) that needs up-to-date
        complainant contact/location info.
        """
        if not grievance_id:
            return [
        ...
        complainant_data = self.db_manager.get_complainant_data_by_grievance_id(
            grievance_id
        )
        ...
        is_valid = bool(phone and self.helpers.is_valid_phone(phone))
