"""
Enhanced logging configuration for Rasa actions.

This provides better logging control with file output, rotation, and structured logging.
"""
...
def setup_rasa_actions_logging(
    log_level: str = "INFO",
    ...
    """
    Set up comprehensive logging for Rasa actions.
    ...
    if log_to_file:
        ...
        # Backend repository specific log
        repo_log_file = os.path.join(log_directory, "backend_repository.log")
        repo_handler = logging.handlers.RotatingFileHandler(
            repo_log_file,
            maxBytes=5*1024*1024,  # 5MB
            backupCount=3
        )
        repo_handler.setLevel(numeric_level)
        repo_handler.setFormatter(detailed_formatter)
        
        # Create repository logger
        repo_logger = logging.getLogger("backend.actions.backend_repository")
        repo_logger.addHandler(repo_handler)
        repo_logger.setLevel(numeric_level)

    # Set specific logger levels
    logging.getLogger("backend.actions").setLevel(numeric_level)
...
def log_action_execution(action_name: str,  data: Dict[str, Any] = None) -> None:
    """
    ...
    """
    logger = logging.getLogger("backend.actions.backend_repository")
...
def log_backend_operation(operation: str, success: bool, details: Dict[str, Any] = None) -> None:
    """
