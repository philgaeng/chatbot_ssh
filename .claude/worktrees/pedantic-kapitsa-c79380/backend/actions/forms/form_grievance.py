...
class ValidateFormGrievance(BaseFormValidationAction):# Use the singleton instance directly
    ...
    async def _get_sensitive_issue_slots_on_submit(
        ...
        # Use keyword fallback whenever DB did not return True (task not run yet, failed, or said False).
        # We do not treat DB False as authoritative here, so tests without Celery still get keyword detection.
        self.logger.debug(
