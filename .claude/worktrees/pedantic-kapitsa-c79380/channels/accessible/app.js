...
const stepWindowList = Object.entries(windowToRecordingTypeMap).map(
  ([k, v]) => [k, Object.keys(v)]
);
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    UIModule.Overlay &&
    typeof UIModule.Overlay.checkForDuplicateOverlays === "function"
  ) {
...
(UIModule = {
  ...
  Navigation: {
    showCurrentWindow: function () {
      ...
      try {
        ...
        // Debug logging
        console.log(
          `[NAV] showCurrentWindow: step='${currentStepKey}', window='${currentWindow}', stepIndex=${UIModule.currentStepIndex}, windowIndex=${UIModule.currentWindowIndex}`
        );
        ...
        if (el) {
          ...
          const recordingType = getRecordingTypeForWindow(
            currentStepKey,
            currentWindow
          );
          ...
          if (recordingType && !requiredRecordings.includes(recordingType)) {
            ...
            if (h2 && navRow) {
              ...
              msg.textContent =
                "Field not mandatory - You can skip by pressing next";
          ...
          console.log(
            `[NAV] Before changes - hidden: ${el.hidden}, display: ${el.style.display}`
          );
          el.hidden = false;
          el.style.display = "block";
          console.log(
            `[NAV] After changes - hidden: ${el.hidden}, display: ${el.style.display}`
          );
          ...
          if (
            ...
            console.log(
              `[NAV] DEBUGGING ZERO DIMENSIONS for element:`,
              elementId
            );
            ...
            contentElements.forEach((child, index) => {
              ...
              if (childRect.width > 0 || childRect.height > 0) {
                console.log(
                  `[NAV] Child ${index} (${child.tagName}.${child.className}) has dimensions:`,
                  childRect
                );
              }
            });
            ...
            console.log(
              `[NAV] Element textContent length:`,
              el.textContent.length
            );
            console.log(
              `[NAV] Element textContent preview:`,
          ...
          if (currentStepKey === "review") {
            ...
            if (window.debugReviewData) {
              ...
              ReviewDataModule &&
              typeof ReviewDataModule.updateReviewUI === "function"
            ) {
            ...
            if (
              reviewModule &&
              typeof reviewModule.updateReviewUI === "function"
            ) {
              console.log(
                "[NAV] Review data available:",
              ...
              console.warn(
                "[NAV] ReviewDataModule not available or missing updateReviewUI function"
              );
              console.log(
                "[NAV] Available on window:",
                Object.keys(window).filter((k) => k.includes("Review"))
              );
            }
          }
          ...
          if (currentStepKey === "outro") {
            ...
            try {
                ...
                try {
                  if (saved) {
                    try {
                      ...
                      console.warn(
                        "[WARN] Failed to parse reviewData from localStorage, using empty object.",
                        e
                      );
                    }
                    try {
                      window.reviewData =
                        ...
                        !Array.isArray(parsed)
                          ...
                          : {};
                      console.log(
                        "[DEBUG] Restored reviewData from localStorage:",
                        window.reviewData
                      );
                    } catch (e) {
                      window.reviewData = {};
                    ...
                    try {
                      window.reviewData = {};
                      console.warn(
                        "[WARN] No reviewData found in localStorage, using empty object."
                      );
                    } catch (e) {
                      console.warn(
                        "[WARN] Failed to restore reviewData from localStorage, using empty object.",
                        e
                      );
                    }
                  }
              ...
              console.warn(
                "[WARN] Failed to get reviewData from localStorage",
                e
              );
            }
            // Defensive type check after restore
            console.log(
              "[DEBUG] After restore, typeof window.reviewData:",
              typeof window.reviewData,
              window.reviewData
            );
            if (
              typeof window.reviewData !== "object" ||
              window.reviewData === null ||
              Array.isArray(window.reviewData)
            ) {
              window.reviewData = {};
              console.warn(
                "[WARN] window.reviewData was not an object after restore, reset to empty object."
              );
            }
            // Optionally clear reviewData from localStorage after rendering
            localStorage.removeItem("reviewData");
            console.log(
              "[DEBUG] window.reviewData at OUTRO:",
              window.reviewData
            );
            const outroFieldMap = {
            ...
            document.getElementById("expectedResolutionDate").textContent =
              dateStr;
    ...
    goToNextWindow: function () {
      ...
      if (UIModule.currentWindowIndex < currentStep.windows.length - 1) {
        ...
        if (currentStepKey === "review") {
          ...
          const openEditAreas = document.querySelectorAll(
            ".edit-area:not([hidden])"
          );
          if (openEditAreas.length > 0) {
            UIModule.showMessage(
              "You have unsaved changes. Please save or cancel your edits before continuing.",
              true
            );
            return;
          }
    ...
    goToNextStep: function () {
      ...
      UIModule.currentWindowIndex = 0;
      console.log(
        "[NAV] goToNextStep - navigation done showing window for step:",
        nextStepKey
      );
      this.showCurrentWindow();
    },
  ...
  Overlay: {
    setupOverlayObserver: function () {
      ...
      if (!overlay) {
        console.error(
          "[ERROR] Could not set up overlay observer: overlay not found"
        );
        return;
      }
    ...
    checkForDuplicateOverlays: function () {
      ...
      if (overlays.length > 1) {
        console.error(
          "[ERROR] Multiple submission overlays found:",
          overlays.length
        );
    ...
    showAttachmentsStep: function () {
      this.hideAllSteps();
      const attachmentsElement = document.getElementById(
  ...
  updateButtonStates: function (options = {}) {
    ...
    navButtons.forEach((button) => {
      ...
      switch (action) {
        case "prev":
          button.disabled =
            step === "grievance" && window === "grievanceDetails";
        ...
        case "submit":
          if (step === "personalInfo" && window === "address") {
            ...
            if (!validation.canSubmit) {
              ...
              button.setAttribute(
                "aria-label",
                `Submit (disabled: ${validation.reason})`
              );
            } else {
  ...
  (FileUploadModule = {
    ...
    init: function () {
      ...
      socket.on(
        "status_update:file_upload",
        this.handleStatusUpdateFileUpload.bind(this)
      );
    ...
    setupAttachmentButtons: function () {
      ...
      if (attachFilesBtn) {
        ...
        attachFilesBtn.parentNode.replaceChild(
          newAttachFilesBtn,
          attachFilesBtn
        );
      ...
      if (submitFilesBtn) {
        ...
        submitFilesBtn.parentNode.replaceChild(
          newSubmitFilesBtn,
          submitFilesBtn
        );
    ...
    handleFileSelection: function (fileList) {
      ...
      SpeechModule.speak(
        `${validFiles.length} files selected. Review the files and click Submit Files when ready.`
      );
    },
    ...
    updateFileList: function () {
      ...
      const listElement =
        fileListElement.querySelector("ul") || document.createElement("ul");
    ...
    submitSelectedFiles: async function () {
      if (!state.grievanceId) {
        UIModule.showMessage(
          "No grievance ID found. Cannot upload files.",
          true
        );
        return;
      }
      ...
      try {
        ...
        if (uploadResult.success) {
          ...
          if (successMessage) {
            successMessage.textContent = `${this.selectedFiles.length} ${
          ...
          // Announce success
          SpeechModule.speak(
            `${uploadResult.results.length} files uploaded successfully. You can attach more files if needed.`
          );
        } else {
          UIModule.showMessage(
    ...
    updateUploadedFilesList: function () {
      ...
      const listElement =
        uploadedFilesList.querySelector("ul") || document.createElement("ul");
    ...
    uploadFiles: async function (grievanceId) {
      ...
      try {
        ...
        for (const file of this.selectedFiles) {
          ...
          console.log(
            `Adding file to FormData: ${file.name}, size: ${file.size} bytes`
          );
        }

        // Upload all files in a single request
        console.log(
          `Uploading ${this.selectedFiles.length} files for grievance ID: ${grievanceId}`
        );
        ...
        if (
          ...
          if (result.status === "processing") {
            UIModule.showMessage(
              "Files are being processed. You will be notified when processing is complete.",
              false
            );
            if (result.warning) {
          ...
          // Handle error
          throw new Error(
            result.error || result.message || "Failed to upload files"
          );
        }
      } catch (error) {
        console.error("Error uploading files:", error);
...
RecordingModule = {
  ...
  requestMicrophonePermission: function () {
    ...
    navigator.mediaDevices
      ...
      .catch((error) => {
        console.warn(
          "Failed to get microphone permission during initialization:",
          error
        );
  ...
  startRecording: async function (recordingType) {
    ...
    if (recordBtn) {
      ...
      recordBtn.textContent = this.permissionGranted
        ? "Preparing recording..."
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `MediaRecorder not available with specified options: ${e.message}`
        );
      ...
      if (statusEl) {
        statusEl.innerHTML =
          '<div class="recording-error">Microphone access denied. Please check permissions and try again.</div>';
      }
  ...
  startTimer: function (recordingType) {
    ...
    this.timerInterval = setInterval(() => {
      ...
      const timeText = `${minutes}:${remainingSeconds
        .toString()
        .padStart(2, "0")}`;
  ...
  stopRecording: function () {
    ...
    console.log(
      `Stopped recording for step ${step} - window ${window}, waiting for user action.`
    );
...
GrievanceModule = {
  ...
  setSubmissionState: function (isSubmitting) {
    ...
    if (oldState !== isSubmitting) {
      ...
      console.log(
        `[STATE] Submission state changed: ${oldState} → ${isSubmitting}`
      );
    }
  },
  ...
  canSubmit: function () {
    ...
    const { step, window } = UIModule.getCurrentWindow();
    if (!(step === "personalInfo" && window === "address")) {
      console.warn("[VALIDATION] Cannot submit: not at final step", {
        step,
        window,
      });
    ...
    const missingRecordings = requiredRecordings.filter(
      (type) => !RecordingModule.hasRecording(type)
    );
    ...
    try {
      ...
      if (result.success) {
        ...
        const successMessage =
          ...
          state.grievanceId.split("").join(" ") +
          ". You can now attach photos or documents.";
        ...
        console.warn(
          `[WARN] Submission failed [${result.type}]: ${result.error}`
        );
  ...
  submitGrievance: async function (validationResult = null) {
    ...
    try {
      ...
      const result = await APIModule.submitGrievance(
        {},
        recordedBlobs,
        formData
      );
      if (result.status === "success") {
        ...
        const errorMessage =
          result.error || result.message || "Failed to submit grievance";
        return {
          success: false,
          error: errorMessage,
          type: "api",
      ...
      return {
        ...
        error:
          error.message ||
          "There was an error submitting your grievance. Please try again.",
        type: "network",
      };
    } finally {
  ...
  loadReviewData: async function (grievanceId) {
    try {
      ...
      const response = await APIModule.request(
        `/api/grievance/${grievanceId}/review`,
        {
          method: "GET",
        }
      );
      ...
      return {
        success: false,
        error:
          error.message || "Failed to load grievance data. Please try again.",
        type: errorType,
      };
    } finally {
      UIModule.hideLoading();
  ...
  handleSubmissionError: function (result) {
    ...
    switch (type) {
      ...
      case "session":
        ...
        SpeechModule.speak(
          `Session error: ${error}. You may need to refresh the page.`
        );
        break;
      ...
      case "network":
        ...
        SpeechModule.speak(
          `Network error: ${error}. Please check your connection and try again.`
        );
        break;
...
EventModule = {
  ...
  setupModifyButtons: function () {
    ...
    document
      .querySelectorAll(".modify-btn[data-recording-type]")
      .forEach((btn) => {
    ...
    document.querySelectorAll(".nav-btn[data-action]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        try {
          switch (action) {
            ...
            case "retry":
              ...
              if (recordingType) {
                ...
                console.warn(
                  "[EventModule] No recording type found for retry action"
                );
              }
              break;
            default:
          ...
          console.error(
            "[EventModule] Error handling navigation action:",
            action,
            error
          );
  ...
  setupDialogButtons: function () {
    ...
    const enableScreenReaderBtn = document.getElementById(
      "enableScreenReaderBtn"
    );
    const disableScreenReaderBtn = document.getElementById(
      "disableScreenReaderBtn"
    );
    ...
    if (confirmReviewBtn) {
      confirmReviewBtn.addEventListener("click", async () => {
        ...
        if (grievanceId && window._reviewData) {
          const result = await updateReviewData(
            grievanceId,
            window._reviewData
          );
...
console.log(
  "🔍 End of app.js reached - about to set up DOMContentLoaded listener"
);
...
function initializeModules() {
  ...
  try {
    ...
    ReviewDataModule = createReviewDataModule(
      socket,
      UIModule,
      RecordingModule,
      GrievanceModule
...
function setupStateChangeListeners() {
  ...
  window.addEventListener("submissionStateChanged", (event) => {
    ...
    console.log(
      `[StateListener] Submission state: ${previousState} → ${isSubmitting}`
    );
...
function initializeGrievanceSession() {
  try {
    ...
    console.log(
      `Using province: ${province}, district: ${district} for ID generation`
    );
