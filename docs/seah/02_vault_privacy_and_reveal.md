...
- Officer default view is **summary-first**; ciphertext must never render — `ticketing/services/pii_vault.py` scrubs pgcrypto hex values and can broker-decrypt single fields with `DB_ENCRYPTION_KEY` for the standard-flow contact card ("Reveal contact" pattern); SEAH stays masked until vault reveal.
