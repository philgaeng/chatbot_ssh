...
| [`ui/08_resolution_actions_catalog.html`](../ticketing_system/ui/08_resolution_actions_catalog.html) — *"Resolution actions — workflow panel"* | **One place:** a panel in each workflow's editor — up to 8 actions, add from a search or type a new one, and a new local action says what it counts as nationally; the admin never chooses an action's owner. Plus *Belongs to* on every workflow and template. ⚠ **Not built** — wireframe for `GRM-119` / `GRM-121` / `GRM-122` | [`12_workflows_configuration.md`](../ticketing_system/12_workflows_configuration.md) — its *Resolution actions* section is written by `GRM-119` when built; until then the items in [`SPINE.md`](../SPINE.md) |
