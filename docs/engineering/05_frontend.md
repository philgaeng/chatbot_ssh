...
| [`ui/08_resolution_actions_catalog.html`](../ticketing_system/ui/08_resolution_actions_catalog.html) — *"Resolution actions — workflow panel"* | **One place:** a panel in each workflow's editor — up to 8 actions, add from a search or type a new one, and a new local action says what it counts as nationally; the admin never chooses an action's owner. Plus *Belongs to* on every workflow and template. ✅ **Built** (frames 1–4): the panel and dialog (`GRM-119`), *Belongs to* (`GRM-122`). ⚠ **Not built:** the similarity suggestion in frame 2b (`GRM-121`) | [`12_workflows_configuration.md`](../ticketing_system/12_workflows_configuration.md) — *The resolution panel* and *A workflow's organization* |
