...
**Last updated:** 2026-09-04 — created. First standard for how work is born, classified and routed.
**Audience:** public.
...
4. **Are we choosing not to fix it now?** Yes → **Debt** — and the standing deferral rule applies: `followups/<slug>.md` **and** a register row, in the same commit ([`../sprints/README.md`](../sprints/README.md)).
...
| **G-RELEASE** | anything deployed | migration order; rollback note; staging before production; smoke check | [`../deployment/03_operations.md`](../deployment/03_operations.md) |
...
⚠ **This standard is adopted and not yet in force.** Everything below is true on 2026-09-04 and is the work of the operating-model sprint ([`../sprints/2026-09_operating_model/`](../sprints/2026-09_operating_model/)).
...
| 8 | **No tracker decision has been taken.** Rule 9.3 describes the shape; the choice is gated on `D-002` (the working repository is public today, so issues would be world-readable on a platform that receives SEAH reports). | OM-07 |
