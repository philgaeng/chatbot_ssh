...
- **Edit** applies to the **whole officer** (all roles and scopes in one modal). **Delete** on the roster row removes the officer entirely (all `user_roles`, all `officer_scopes`, Keycloak disable on auth stack). **Per-role** and **per-scope** delete buttons live **inside** the modal when multiple rows exist.
...
| **Edit** | Same fields + list of roles/scopes with per-row delete. Keycloak sync on auth. |
| **Delete** (row) | Remove all roles/scopes; disable Keycloak user; audit log. |
| **▶ Scopes** (optional) | May remain as quick view; full edit in modal. |

Columns may show aggregated project/package from scopes (not only `user_roles`).
...
- [ ] **UI:** Invite/Edit modals with required jurisdiction; Edit/Delete on roster; per-role/scope delete in modal
