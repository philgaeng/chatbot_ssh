...
- Officer chooses **what was done** — one of the **resolution actions its workflow offers** (§2.2) — and enters **resolution text**. Both are required, **except on a case in a sensitive workflow**, which records no action: the text alone is required, and sending an action is refused.
...
### 2.2 Resolution actions — one catalog, selected per workflow (`GRM-116`)

**What an officer can record as done depends on the case's workflow.** The actions live in one
catalog, `ticketing.resolution_actions`, and each workflow holds an ordered selection of them,
`ticketing.workflow_resolution_actions` ([04](04_ticketing_schema.md)). A case offers the selection
of **the workflow it is in** (`tickets.current_workflow_id`) at the moment of resolving, so a case
re-classified into another workflow offers that workflow's actions.

**Why one catalog and not a list per workflow.** A list copied onto each workflow puts the same
action under several codes and lets its wording drift apart between copies — so duplicates cannot be
caught, and outcomes cannot be totalled across workflows. One catalog gives an action one code
everywhere. Codes are the primary key: platform-unique, immutable, never reused. The API field and
event key stay `resolution_category` (they are persisted in append-only events and in saved report
templates); on screen the word is *what was done*.

**Nothing is global — every action and every workflow belongs to one organization** (the org-scoped
catalog rule of [11 §3.3](11_roles_and_permissions.md), with global items removed for this kind):
...
**Rules, enforced in `ticketing/services/resolution_catalog.py`** — the only writer of lists and the
only way to create an action, so no caller can skip them:

1. an action the workflow's organization cannot use is refused;
2. **a sensitive workflow lists nothing** — a SEAH case records neither what was done nor who did it:
   a structured value about a SEAH case in a report re-identifies, and before this a SEAH case was
   *forced* to pick a general outcome such as *Complainant demand rejected*;
3. **at most 8 actions** per workflow and template (`MAX_RESOLUTION_ACTIONS`, a constant): an officer
   chooses from a short list, often on a phone, in a second language;
4. a **published** non-sensitive workflow lists **at least one**. A draft or template may be empty,
   and `POST /workflows/{id}/publish` refuses a non-sensitive workflow with none — *"Add at least one
   resolution action before publishing."* Only a published workflow can be bound to a project, so a
   case never meets an empty list unless its workflow is sensitive;
5. a local action without `counts_as_code`, a shared action with one, or a `counts_as_code` that is
   not a shared action of the same ministry is refused.

**Starter actions** — seeded by migration `b3d5f7h9` as **shared actions of the ministry that
implements projects** (`DOR` on every database today); on an empty database the seed scripts create
them under `DOR`. The general five keep their pre-catalog codes and wording, so every historical event
still resolves to a label.

| Code | Label | Default wording (EN) |
| --- | --- | --- |
...
| `ACCEPTED_OTHER` | Grievance accepted — other remedy | The grievance is substantiated. Remedial action has been agreed (other than monetary compensation or relocation). Details are recorded below. |
...
**Where a workflow's list comes from.**
...
**A list is copied, never inherited**: a later change to the source does not reach the copy. With
inheritance, a shared list growing past a full local list would have to block the ministry, break the
limit of 8, or silently drop local actions; a copy never forces that choice.

⚠ **A project that wants road-works actions must bind a road-hazard workflow.** A road-hazard report
...
| ----- | ------------ | ----------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------- |
| 1     | `NOTE_ADDED` | Thread bubble — “resolution record” | Formatted resolution body (see §2.4)           | `{ "internal": true, "is_resolution_record": true, "resolution_category": "<code>", "resolution_category_label": "<label>" }` |
| 2     | `RESOLVED`   | Status change + system pill         | `Case resolved — <label>` (or `Case resolved`) | `{ "resolution_category": "<code>", "resolution_category_label": "<label>", "resolution_event_id": "<uuid of row 1>" }`       |

**`resolution_category_label` is a snapshot** of the action's label at resolve time (`GRM-116`).
Readers — reports, the Summary tab, the resolved case summary, the thread pill — use it first, and
...
default_wording}]` — the active actions the case's workflow selects, in order; **empty for a sensitive
...
- **What was done** `<select>` — the ticket's `resolution_options`. Preselects `ACCEPTED_OTHER` when
...
- System pill shows `Case resolved — <label>` from the `RESOLVED` event's snapshot (the pre-catalog five by code for older events), or `Case resolved` when no action was recorded.
...
| Outcome | Resolution action label (plain language) — **omitted for a case that recorded none** (sensitive workflow, `GRM-116`); **resolution text for complainant** | Officer roster, field-report digests, internal notes |
