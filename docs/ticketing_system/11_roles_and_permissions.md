...
**Last updated:** 2026-09-15 — §3.3: resolution actions become a fifth org-scoped catalog kind — **the one with no global items** — with the availability rule applied to the workflow, and templates now owned like workflows (`GRM-116`); the platform admin chooses the organization and admins move workflows between organizations they manage (`GRM-122`); authoring resolution actions needs reach on the workflow's or the action's organization (`GRM-119`). Earlier: 2026-09-04 — sprint citations folded — reasons kept inline, forks recorded in `DECISIONS.md` (lifecycle §10) · ⚠ header date backfilled; content not re-verified against the code
...
**Org-scoped catalog (as-built 2026-07).** Each catalog item (role, workflow definition, position type — and, since `GRM-116`, resolution action) carries an **`owner_organization_id`** and is available **only at its owning org node and below** (`owner IS NULL` = **global**, super-owned; system/TOR seed = global). An `org_admin` authoring at node N creates items **owned by N**, visible to N + `descendant_org_ids(N)` only. This is what makes "org_admin authors at any depth" safe — a district admin's custom role exists only in that district; no global proliferation of N incompatible role keys.

**Resolution actions are the one catalog kind with no global items** ([08 §2.2](08_ticket_resolution_and_case_summary.md), `GRM-116`). Every action belongs to an organization: a **ministry's** actions are shared, an action below it is local and counts as one of its ministry's shared actions. The availability rule applies to the **workflow**, not only the viewer: an action is usable by a workflow whose organization is the action's or below it — so one ministry's vocabulary never reaches another's. That in turn requires **every workflow and template to belong to an organization** — templates included, which were stamped `NULL` before; the migration assigned each ownerless one its projects' ministry. A platform admin **chooses** a new workflow's or template's organization; an `org_admin`'s defaults to its own and must be in reach; moving one needs reach on both organizations ([12](12_workflows_configuration.md), `GRM-122`). **Authoring resolution actions** happens in the workflow's panel ([12](12_workflows_configuration.md), `GRM-119`): changing a list needs the **workflow's** organization in reach; editing an action needs the **action's** organization in reach; a *national* action needs the ministry in reach. The admin never chooses an owner. ⚠ These endpoints check reach, not only the track — the track check elsewhere in the workflow editor lets any standard-track `org_admin` through (`GRM-120`).
