...
**Last updated:** 2026-09-15 — §3.3: resolution actions become a fifth org-scoped catalog kind — **the one with no global items** — with the availability rule applied to the workflow, and templates now owned like workflows (`GRM-116`). Earlier: 2026-09-04 — sprint citations folded — reasons kept inline, forks recorded in `DECISIONS.md` (lifecycle §10) · ⚠ header date backfilled; content not re-verified against the code
