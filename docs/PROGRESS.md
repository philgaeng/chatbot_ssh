...
### What's done
- ✅ **The spec tree is ready for the public/private split — and the header rule finally has an enforcement point** (2026-09-04). Rule 6.1 ("dated Status header on every doc") had existed since 2026-08-03 with nothing enforcing it: **40 of 80 live specs carried no header at all**, while `docs/engineering/` — the folder the rule lives in — scored **7 of 7**. Now: `scripts/ops/doc_headers.py` (`--check` / `--stamp` / `--provenance`) + 9 tests in `tests/repo/test_doc_headers.py`, and CI checks out with `fetch-depth: 0` because the history rule needs more than a shallow clone. **81 headers backfilled with their *last-commit* date, not today's**, each marked `⚠ backfilled … not re-verified` — stamping 81 documents "reviewed today" would have been 81 claims nobody made. ⭐ **The commit-hash question is answered by not storing one:** a hash cannot be written into the content it describes, so the header carries what git cannot infer and `--provenance` derives the rest. **Sprint citations folded out of every public spec: 93 links across 28 files → 0**, with the four org-model forks and the `public.*` boundary promoted to `docs/DECISIONS.md` (D-004…D-008) rather than deleted — *a spec that loses a pointer and gains nothing has been damaged, not cleaned*. 🔴 **And it caught a document that must never be published:** `deployment/18_sops_migration_handover.md` carries the staging host address, an `ssh` login and **the live pre-rotation database credential** — load-bearing to its argument, so it is marked `Audience: internal` rather than scrubbed.
  ⚠ **Not done, and not mistaken for done:** pruning `docs/` does **not** make the tree publishable. The staging IP is in `Makefile`, `deployment/nginx/webchat_rest_compose_aws.conf`, `scripts/ops/aws_to_prod_db_sync.sh` and the tracked `.claude/settings.local.json`; the EC2 instance id is in `docs/TODO.md`. **A publish filter that only looks at `docs/` gives false confidence.** The public repo and its prune script are deliberately out of scope here.
- ✅ **Sprints 2 and 3 are live on AWS staging** (2026-09-03) — and the first deploy only shipped **half** of them, which is the part worth remembering. `AWS_DEPLOY_SERVICES` covers `ticketing_api backend celery_default grm_celery grm_celery_beat ops grm_ui`; it does **not** include `orchestrator`, `celery_llm` or `celery_file` — the three containers that actually run the intake path and the model calls. So `aws-deploy OK` printed while every line of the PII sprint sat undeployed on 5-week-old containers. ⚠ **Nothing was broken by that** — old dispatch and old worker were consistent with each other — but "the deploy succeeded" and "the change is live" were not the same sentence. Completed with two ordered deploys, **worker before orchestrator**, because the new dispatch sends only `grievance_id` and an old worker would have had no text to classify (the DB-read fallback covers the reverse order, which is why that order is the safe one). **Verified by exercising the code in the containers, not by listing files:** redaction returns `Er. <PERSON_1> refused to spray. Call <PHONE_1>, <ADDRESS_1> Duhabi.` with an exact round-trip restore, the log filter is installed on `TaskLogger`, `OTP_VALIDITY_SECONDS=600` with a missing stamp failing **closed**, both LLM tasks registered, `/health` 200, zero errors since restart. ⭐ **`Duhabi` surviving is the documented residual reproducing outside dev** — bare settlement names are not caught, and *names removed is not identity removed*.
- ✅ **The ops monitor was blind for three days, and the daily report had never worked at all** (2026-08-24, local stack). Found by *running* the report queries while verifying the Keycloak fix, rather than reading them. Four layers, outermost first: **(1)** `ops` had no credential of its own — `ops/config.py` fell back to `POSTGRES_PASSWORD` while connecting as the **`ops_app`** role, and the 2026-08-21 rotation did not include `ops_app`; 253 auth failures, every row `n/a`, container reporting `healthy`. **(2)** `_safe_scalar` caught query errors without `db.rollback()`, so Postgres's aborted transaction made **one bad query blank every row after it** — the degradation was meant to be per-row and was per-report. **(3)** Four queries named columns that do not exist: `grievances.created_at` (it is `grievance_creation_date`), `tickets.status` (`status_code`, upper-case vocabulary), `ticket_overdue_episodes.created_at` (`started_at`), `ticket_files.created_at` (`uploaded_at`). **(4)** `ops_app` lacked SELECT on five tables it reads, including the entire `keycloak` schema — and `ticketing.tickets` *was* in `ops001`'s list, behind an `IF to_regclass(...)` guard that **silently skips** when ops migrates before ticketing. Fixed: own password in `secrets.enc.env` (45 vars in `env.local`), the rollback, the four queries, migration **`ops003_report_grants`** (whose guard now `RAISE WARNING`s what it skipped), and a healthcheck that probes the database and names the failing role — **red-tested by re-breaking the credential**, exit 1 where it used to say `healthy`. All eleven report rows now return numbers cross-checked against ground truth. ⭐ **And it happened a second time on 2026-09-03, on staging, from a different cause** — `ops` is in `AWS_DEPLOY_SERVICES`, so the ordinary deploy shipped it to a host with nobody deciding to deploy `ops`, and `OPS_DB_PASSWORD` had never reached that box (`make env-local` is unsafe there — it would delete thirty host-only variables including the Nepal SMS gateway token). Same silent fallback, **`health_rows=0` for five hours, 309 failing checks, every job logging `executed successfully`.** Fixed the same day per [`14_… §5.1`](deployment/14_key_and_secret_lifecycle.md): variable appended without printing it (digest-verified, `67 → 68` names), `ALTER ROLE ops_app`, container recreated, `selfcheck` exit 0, then the evidence that actually settles it — the **first five health rows ever written on that host**, all `ok`. ⚠ **The written runbook did not prevent it**: it was gated on *"if `ops` is ever deployed to a host"* while two documents asserted `ops` ran on neither server — **a precondition enforced only by prose has a shelf life**. Two follow-ups logged: make the deploy run `selfcheck`, and make the password fallback fatal. ⚠ **DOR prod still does not run `ops`** ([`sprints/2026-08-llm/followups/ops-cannot-authenticate-since-rotation.md`](sprints/2026-08-llm/followups/ops-cannot-authenticate-since-rotation.md)).
- ✅ **A breach procedure exists, and writing it caught a live monitoring defect** (2026-08-24). [`deployment/19_incident_response.md`](deployment/19_incident_response.md) — detection, triage by what each store actually holds, containment **in order** (with the two silent-failure traps from the rotation table, and the warning that `docker compose down` deletes the container logs of the incident you are investigating), evidence with its retention clocks, notification, post-incident review. **Three decisions are blank by design** — who declares a breach, who is notified on what clock, and how a survivor is told when a SEAH case is exposed — because they are the implementing agency's; **Philippe holds all three interim** and the document says so rather than implying an arrangement nobody agreed to. `SECURITY.md` now points at it instead of at a promise with nothing behind it, and warns the reporter which parts are uncommitted. ⭐ **The defect it found: Keycloak was recording no login, login-failure or admin events at all** — realm event storage defaults to off and nothing turned it on (`events_enabled=f`, `event_entity` empty). It survived every review because `ops/reports.py:45,55` queried that empty table and printed **0 logins** — indistinguishable from a quiet day. Fixed by `setup_realm_event_logging` (90-day expiration, verified end-to-end: a failed login now writes `LOGIN_ERROR`). ⚠ **Forward-only — nothing before 2026-08-24 is recoverable** — and ⚠ **not yet applied to staging or prod** ([`sprints/2026-08-llm/followups/keycloak-recorded-no-login-events.md`](sprints/2026-08-llm/followups/keycloak-recorded-no-login-events.md)). Privacy assessment **F-8** downgraded, **F-18** logged and closed.
- ✅ **Secrets are encrypted at rest, and `env.local` is now a generated artefact** (2026-08-21, local only — `make wsl-up` **14/14 containers healthy**). `.env.shared` (32 vars, committed plaintext, keeps every comment and section header) + `secrets.enc.env` (11 vars, SOPS + age, committed) → `env.local` via `make env-local` (`scripts/ops/gen_env_local.sh`, 0600, idempotent, fails loudly when the two halves disagree in either direction). Regenerated `env.local` carries **all 43 variable names and every value byte-identical** to the pre-migration file. The encrypted half was defined by a **restatable rule** — this project's own secret inventory ([`deployment/13_security.md`](deployment/13_security.md) §5.3.1) ∩ `env.local` — not a hand-picked list, which is why it is 11 and not the handover's 8. **Two inventories became one:** [`deployment/14_key_and_secret_lifecycle.md`](deployment/14_key_and_secret_lifecycle.md) §1 is now the single authority (15 rows: impact, cadence, procedure, `Last rotated`), §5.3.1 keeps location columns and points at it — *two rotation procedures for one credential is how the wrong one gets followed during an incident*. ⚠ **The plaintext half is `.env.shared`, not `.env`** — `.env` is gitignored deliberately (CL-03) and un-ignoring it to satisfy a filename would delete the guard against the universal "secrets go in `.env`" reflex. ⏸ **Not done:** the age key is **not yet backed up** (do this first — a disk failure makes `secrets.enc.env` unreadable), no credential rotated, and **neither staging nor DOR prod migrated** — six secrets live only there. Handover: [`deployment/18_sops_migration_handover.md`](deployment/18_sops_migration_handover.md)
- 🔴 **Found while doing it: `POSTGRES_PASSWORD` is hardcoded as `password` in 11 compose sites, and the staging/prod overlays do not override it** (2026-08-21). Compose `environment:` **beats `env_file:`**, so the value just encrypted **is read by nothing** — verified live: the `db` container reports `user=user` while `env.local` says `nepal_grievance_admin`. `db` publishes **`0.0.0.0:5433`** on every GRM host, leaving the firewall as the single control. `security-preflight.sh:61` asserts the password is non-default **against the inert copy**, so the promotion gate is green on a variable nothing consumes. **T3-08 found half of this in July** and closed it as a test-fixture problem. ⚠ **Do not rotate `POSTGRES_PASSWORD` until it is fixed** — rotating an inert variable writes a false `Last rotated` date. ⚠ **And the live `POSTGRES_PASSWORD` is byte-identical to a literal committed in six tracked files** — including `.claude/settings.local.json`, a tracked permission rule embedding it in a `PGPASSWORD=` command — so encrypting it protects nothing until it is **rotated** and the literal removed. `SMTP_USERNAME` is similarly committed in three places. `REDIS_PASSWORD` is correctly wired and **is** protected (verified live). Fix order: [`sprints/followups/db-password-hardcoded-in-compose.md`](sprints/followups/db-password-hardcoded-in-compose.md)
- ✅ **DPG Sprint 0 — the platform is submittable on licensing, and honest about privacy** (2026-08-18, branch `dpg/sprint0-licensing`, `tests/repo` 46 passed). **DPG-01:** Apache-2.0 `LICENSE` + `NOTICE` with an explicit `⚠ PENDING` copyright holder (not guessed), **585 SPDX headers** maintained by `scripts/ops/add_spdx_headers.py` and pinned by test. **DPG-02:** `docs/dpg/dependency-licenses.md` — 153 packages across four dependency sets scanned **in-container**, zero unknown or non-OSI, and the scan now runs **nightly** in `ops/security.py` beside `pip-audit`. **DPG-04:** `docs/dpg/privacy-assessment.md` — 13 data-flow legs verified at file and line, assessed against Nepal's Individual Privacy Act 2018, **17 findings** including three nobody knew about: encryption at rest **fails open** when `DB_ENCRYPTION_KEY` is unset or pgcrypto raises (`base_manager.py:266-297`), `*_hash` search tokens are **unsalted SHA-256** of phone/email/name/address so the phone hash is reversible (`:559-573`), and `backup_db.sh` writes an **unencrypted** dump + uploads tar by default. It carries a mandatory honesty marker: drafted by an AI agent, **no legal review**. **DPG-05:** `SECURITY.md` (private channel — this platform holds SEAH reports), `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `.github` templates with `blank_issues_enabled: false`. **DPG-06:** root `README.md` rewritten from the compose files — the Rasa service on :5005 and the Action Server on :5055 never existed, and neither did any of the three environment URLs it advertised. ⏸ **DPG-03 (IP ownership) is blocked on ADB's Office of the General Counsel** and named as a clock in the sprint tracker
