...
| `POSTGRES_PASSWORD` | Postgres; all services | DB outage | Full DB access | 12 months / on personnel change | `ALTER ROLE … PASSWORD`, `make secrets-edit`, `make env-local`, restart the stack | unknown — treat as never |
...
| `REDIS_PASSWORD` | Celery broker/result + Socket.IO bus | Broker outage | Task injection / inspection | 6–12 months / on suspicion | Update `requirepass`, `make secrets-edit`, `make env-local`, restart Redis **and every worker** (URLs pick it up via `${REDIS_PASSWORD}`) | unknown — treat as never |
