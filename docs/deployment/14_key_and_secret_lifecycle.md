...
**Last updated:** 2026-09-17 — ✅ **`POSTGRES_PASSWORD` and `REDIS_PASSWORD` rotated on DOR production** (§1 rows 1 and 3), retiring the `password` credential three tracked files still name; ⚠ **AWS staging remains un-rotated for both.** Earlier, 2026-09-04 — sprint citations folded — reasons kept inline, forks recorded in `DECISIONS.md` (lifecycle §10) · ⚠ header date backfilled; content not re-verified against the code
