...
| `grievance_sync.py` hardcoded column list | `tasks/grievance_sync.py` | Will break if public schema column names change — add integration test. **Same family as the cross-schema direct reads below — consider folding into that ticket.** |
...
| Ticketing reads `public.grievances` (incl. the raw narrative) + `public.file_attachments` **directly via its own DB session** — violates locked data rules #1/#5 | `ticketing/services/grievance_content.py:22-38`, `routers/tickets/files.py:91`, `api/ticket_access.py:167` | Deferred out of T3-04 (different concern: retrieval path vs decryption responsibility). **Means T3-04 alone does not fully restore the "single auditable PII boundary"** — the sprint summary must not overclaim. Needs a locked-architecture **decision** (route via API / legitimize a documented read contract / hybrid), not an implementer's judgment. Note the `file_attachments` read is load-bearing for HR-02's authz gate. Tracked: [`sprints/2026-08_tier3_structural/followups/ticketing-cross-schema-direct-reads.md`](sprints/2026-08_tier3_structural/followups/ticketing-cross-schema-direct-reads.md) |
