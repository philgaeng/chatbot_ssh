...
| 🔵 **Roughly a dozen `LLM_services.py:NNN` citations in the docs point at the wrong line — they were stale before the D-51 commit, not because of it** | `docs/dpg/privacy-assessment.md` · `docs/sprints/2026-08-llm/*.md` | Logged 2026-08-25. The D-51 commit re-pointed every citation whose line it moved and updated `ANCHORS`, which is what `tests/repo/test_doc_code_refs.py` enforces. What it did **not** fix: citations that were already wrong, which that test cannot see (property 3 covers only the curated anchors). ⚠ Two are claims, not just numbers — **P-10 and privacy-assessment F-6 say `parse_llm_response` logs the raw model response, and since DPG-13 it logs the length only.** An evidence pack whose whole claim is "verified at the line cited" should not carry either. Fix is a read-and-verify sweep of ~20 citations, or widen `ANCHORS` |
