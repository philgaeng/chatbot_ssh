...
| 🔴 **`POSTGRES_PASSWORD` is hardcoded as `password` in 11 compose sites; the staging/prod overlays do not override it** — so the SOPS-encrypted copy is **read by nothing**, and `db` publishes `0.0.0.0:5433` on every GRM host | `docker-compose.yml` (×7 + 5 `DATABASE_URL`), `docker-compose.grm.yml` (×4), `scripts/ops/security-preflight.sh:61` | Found 2026-08-21 during the SOPS migration ([`deployment/18_sops_migration_handover.md`](deployment/18_sops_migration_handover.md)). Compose `environment:` **overrides `env_file:`** — verified live: the `db` container reports `user=user`, while `env.local` says `POSTGRES_USER=nepal_grievance_admin`. **The promotion gate checks the wrong value**: `security-preflight.sh` asserts `POSTGRES_PASSWORD != "password"` against `env.local`, i.e. the inert copy, and passes. **T3-08 found half of this** (row above: *"env.local's `POSTGRES_*` are dead by construction"*) and closed it as a test-fixture issue — nobody carried it to the question of what the password is on a deployed host. ⚠ **And the live `env.local` `POSTGRES_PASSWORD` is byte-identical to a literal committed in six tracked files** — `backend/config/constants.py:519`, `scripts/database/config.sh:32`, `legacy_rasa_config/endpoints.yml`, `tests/ticketing/test_host_env.py:34`, an archived findings doc, and **`.claude/settings.local.json`** (a tracked permission rule embedding it in a `PGPASSWORD=` command). So encrypting it into `secrets.enc.env` protects nothing until it is **rotated** and the literal removed from all six. `SMTP_USERNAME` is similarly committed in three places. ⚠ **Do not rotate `POSTGRES_PASSWORD` until this is fixed** — rotating an inert variable writes a false `Last rotated` date. `REDIS_PASSWORD` is **not** affected (correct `${...}` interpolation; verified live with `PONG`). Fix order: [`sprints/followups/db-password-hardcoded-in-compose.md`](sprints/followups/db-password-hardcoded-in-compose.md) |
