...
| Authz gaps surfaced by H2-03: (1) 15 unauthenticated reference/config GET endpoints (project-config reads questionable), (2) `DELETE /roles/{id}` no gate on non-system roles, (3) `PATCH /projects/{id}` metadata no gate despite "Admin only" docstring | `ticketing/api/routers/locations.py` (`:1509`, GET reads), `users.py` (`:292`) | H2-03 findings (test-only ticket surfaced them). Tracked ticket: [`sprints/2026-08_tier2_quality/followups/authz-gaps-h2-03.md`](sprints/2026-08_tier2_quality/followups/authz-gaps-h2-03.md). Regression-guarded by the sweep allowlist + a `PATCH /projects` xfail. |
