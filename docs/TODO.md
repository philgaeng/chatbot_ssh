...
| Item | File | Notes |
|------|------|-------|
| **`GET /api/grievance/{id}` and `POST /api/grievance/{id}/status` are unauthenticated** | `backend/api/routers/grievance.py:249`, `:202` | No `Depends` on either, while the two `PATCH`es beside them have `Depends(_ticketing_auth_check)`. The GET returns the full record incl. `grievance_description` (raw narrative); the POST **mutates grievance state and fires SMS + email to the complainant** (`:227`). No read audit exists anywhere in `backend/`. Bound to the host at `docker-compose.grm.yml:117` (`5001:5001`); **not** proxied by prod nginx (no `location /api/grievance`) — so internet reachability depends on the EC2 security group. **⚠️ THAT CHECK IS UNVERIFIED and cannot be answered from the repo — if :5001 is open inbound, treat as an incident.** Tracked: **T3-06** ([`sprints/2026-08_tier3_structural/05-grievance-api-hardening-spec.md`](sprints/2026-08_tier3_structural/05-grievance-api-hardening-spec.md)). Surfaced 2026-07-15 by the §6 boundary reassessment — **no review had examined these endpoints.** |
