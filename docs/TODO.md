...
| `grievance_sync.py` hardcoded column list | `tasks/grievance_sync.py` | Will break if public schema column names change — add integration test. **Absorbed into T3-07** ([`06-boundary-policy-spec.md`](sprints/2026-08_tier3_structural/06-boundary-policy-spec.md) step 2): the contract-drift guard over the enumerated `public.*` reads covers this file. **Close this row when T3-07 lands.** Note this task also `LEFT JOIN`s `public.complainants` (`:178`) — the table the original spec marked *"never touch"* — every 2 min, for `location_code` only. |
