...
| Host `pytest tests/ticketing` fails on **stale `env.local` DB credentials** (`nepal_grievance_admin`/`grievance_db` vs compose's `user`/`app_db`) | `tests/ticketing/_host_env.py:44-55`, `env.local:2-6` | Found while running T3-06's baselines. `_host_env.py` prefers `env.local` values over its own correct `setdefault("user"/"password"/"app_db")` fallbacks, so **108 failed / 234 errors** with `password authentication failed`, which reads as a code regression rather than config. This is the **"dead config" `00-reassessment.md` §6 already identified** (`grievance_db` "is dead config, overridden by every compose `environment:` block") — it is not merely unused, it actively breaks the host test loop. Workaround: `POSTGRES_USER=user POSTGRES_PASSWORD=password POSTGRES_DB=app_db POSTGRES_PORT=5433 pytest tests/ticketing` (the guard at `:54` yields to anything already in `os.environ`). Fix: correct `env.local`, or make `_host_env.py` prefer compose values. Low effort, high friction saved. |
| `tests/ticketing` is **not reliably green on a developer machine** — it depends on unstated DB seed state | `tests/ticketing/test_officer_assignment.py:338,434,475` | Found while running T3-07's baselines (D-44). `TestPackageRouting::test_package_ticket_without_package_officer_falls_back_to_province` asserts the assignee is one of **2** seeded L1 officers; a dev DB with 4 (`l1-officer`, `-2`, `-3`, `-4`) makes load-balancing return `l1-officer-3` ⇒ fail. `l1-officer-3` is in **neither the seed code nor the test** — it is DB residue. Not a code bug and not pollution (fails in isolation). **Impact: the 549/5 baseline every Tier-3 ticket measures against silently depends on a pristinely-seeded DB, and nothing says so** — a real regression and a stale-DB artifact look identical. Fix: assert against the officers the test seeds, or reseed in a fixture. **Same family as the `env.local` credential drift below and the `tests/backend`-not-in-CI row — do all three in one test-hygiene pass.** |
