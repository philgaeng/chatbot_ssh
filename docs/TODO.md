...
| 🟠 **Dependency vulnerabilities: 34 on GitHub are against a 2-month-stale `main`; the real count on this branch is 6 Python + 4 npm-high** | `requirements*.txt`, `channels/ticketing-ui/package.json` | Measured 2026-08-23 in-container ([`dpg/dependency-licenses.md`](dpg/dependency-licenses.md) §Known vulnerabilities). ⭐ **Two of the six Python findings come from `rasa-sdk`, which exists only as a type shim** — it drags `sanic-cors` (a web CORS library, for a web server this repo does not run) and `wheel` into the image. Dropping it removes a third of the findings; contained, but touches live chatbot action signatures. ⚠ **`ecdsa` (via `python-jose`, the Keycloak JWT path) has NO published fix** — the only one not closable by a bump. `sharp`/libvips (4 high) needs a Next.js bump outside the stated range. `python-dotenv` and `setuptools` are trivial. ⚠ **Also: `ops` is not deployed**, so the nightly CVE + licence scans run only in a dev stack — the consultant docs were corrected to say so rather than claim a nightly guarantee. |
