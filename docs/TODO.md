...
| Item | File | Notes |
|------|------|-------|
| `run_flow_turn` **handler table** (T3-02 p4) — not done; the chain is still 20 inline `elif`s | `backend/orchestrator/state_machine.py` | **STRETCH goal, deliberately not started** — and the spec's "S / 22 one-line entries against a proven signature" sizing does not survive measurement (D-53). That is true only if every branch already *has* the signature; **p3 extracted one** (`status_check_form`, 303 lines — correctly, it is where the risk lives), leaving **19 arms / ~1,058 lines still inline** (`done` 120, `modify_grievance_menu` 107, `contact_form` 101, `map_location` 94, `main_menu` 92 …). So p4 is *"extract 19 more branches, then write a table"* — an **M**, not an S; the table is its last 5%. Banked meanwhile: run_flow_turn **1,485 → 1,175 lines (−21%)** via p1+p3, chain intact. ⚠️ **The table does NOT fix D-51/D-52** — those silences are inside *recognized* branches, so the dict lookup succeeds and the default never fires; the `run_flow_turn` postcondition proposed in those followups is cheaper and worth more. Buys reviewability only — do not resell D-46's mistake. Tracked: [`sprints/2026-08_tier3_structural/followups/run-flow-turn-handler-table.md`](sprints/2026-08_tier3_structural/followups/run-flow-turn-handler-table.md) |
| `status_check_form` **silently** treats an unknown `active_loop` as status form 1 ⇒ zero-message turn, session wedged | `backend/orchestrator/state_machine.py:1674-1683` | Found while writing **T3-02 p2**'s characterization tests; **recorded, not fixed**. The form-selection chain ends in a bare `else: form = _get_status_form_1()`; on an already-filled session that form dispatches nothing, and nothing clears the bogus `active_loop`, so **every** later turn repeats the silence. Measured: 0 messages, state unchanged. **Third instance of D-08's dead-air class** (with D-51) — and, like D-51, invisible to T3-02 p1's terminal `else` because the *state* is recognized, only the *loop* is not. ⇒ **Prefer the general fix over three point fixes:** a `run_flow_turn` postcondition that no turn returns zero messages (logging state + active_loop at error) subsumes D-08, D-51 and D-52. Reachability unproven (needs a persisted session naming a retired loop) — the same mechanism that justifies p1's `else`. Wrong behaviour pinned by `test_unknown_active_loop_falls_back_to_status_form_1_silently`: **if it fails, the bug was fixed — delete it, don't relax it.** Tracked: [`sprints/2026-08_tier3_structural/followups/unknown-active-loop-silent-fallback.md`](sprints/2026-08_tier3_structural/followups/unknown-active-loop-silent-fallback.md) |
| ~~**`GET /api/grievance/{id}` and `POST /api/grievance/{id}/status` are unauthenticated**~~ | — | ✅ **RESOLVED 2026-07-15 by T3-06** (4 commits: contract → audit → client key → authn). Both now `Depends(_ticketing_auth_check)`; the GET gained a `response_model` (51 fields, `extra="allow"` so no column is silently dropped) and a read audit at INFO carrying the caller principal. Verified red pre-fix: unauthenticated, the GET returned **200 with `grievance_description`**. Dev bypass + HR-01 fail-closed 503 preserved and pinned. **⚠️ The exposure check is now ANSWERED: `:5001` is NOT admitted** — SG `sg-01739221c73ab3eb1` (`chatbot-main`/`chatbot-rest`, `ap-southeast-1`) allows only 80/443/22(restricted)/8000/3000. **Not an incident; Phase 2 confirmed.** Two caveats carried below: DOR prod is on non-AWS infra and remains unverified, and `tcp/8000 0.0.0.0/0` **is** open to an unauthenticated orchestrator. See [`sprints/2026-08_tier3_structural/05-grievance-api-hardening-spec.md`](sprints/2026-08_tier3_structural/05-grievance-api-hardening-spec.md). |
...
| Item | File | Notes |
|------|------|-------|
| `add_more_info_flow` completes into a **silent turn** when `story_route` is unset | `backend/orchestrator/state_machine.py:1916-1932`, `backend/actions/action_ask_commons.py:28-48` | Found while writing **T3-02 p2**'s characterization tests; **recorded, not fixed** (spec: characterization pins current behaviour, incl. bugs). The completion path invokes `action_ask_story_step`, whose whole body is guarded by `if story_route in [...]` and otherwise dispatches nothing ⇒ the turn returns **zero messages** — the user submits their detail and the chatbot goes quiet. Measured: unset ⇒ 0 messages, set ⇒ 1. **This is D-08's dead-air class one level down** — the state is *recognized*, so T3-02 p1's terminal `else` and p4's handler table both structurally cannot catch it. Reachability **unproven** (the real status-check flow sets `story_route` first) — trace before dismissing; D-13 is the precedent for reachability guesses being wrong on this codebase. The wrong behaviour is pinned by `test_add_more_info_submit_is_silent_when_story_route_is_unset`: **if it ever fails, the bug was fixed — delete the test, don't relax it.** Worth considering the general guard (a `run_flow_turn` postcondition that no turn returns zero messages) over the point fix. Tracked: [`sprints/2026-08_tier3_structural/followups/add-more-info-silent-turn.md`](sprints/2026-08_tier3_structural/followups/add-more-info-silent-turn.md) |
