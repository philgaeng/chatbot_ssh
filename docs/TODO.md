...
| Item | File | Notes |
|------|------|-------|
| 🔴 **Demo officer switcher is a one-way door — blocks the browser sweep's §C4** | `channels/ticketing-ui/app/providers/AuthProvider.tsx:264-269` · `ticketing/api/routers/users.py:976` | Switch to any **non-admin** officer and you cannot switch back: the switcher's roster (`GET /users/roster`) is gated on `require_admin`, so the control depends on the privileges of the identity it switches *away from*. The catch branch then sets a hardcoded `super_admin` display token **without clearing the cookie** (its sibling at `:260-263` does clear it) ⇒ the **UI reports "GRM Admin" while every API call still carries the officer's identity** ⇒ admin-looking UI over officer-scoped data, i.e. "my grievances vanished". **Not low-urgency for UAT:** it blocks `17_manual_browser_sweep.md` §C4 — the section the sweep calls the one to run if you only run one. **Bypass builds only; not a production authz hole** (Keycloak ignores the cookie, `route.ts:54-56`). **Escape hatch** (now in §C4): clear `grm_bypass_user` + reload. Smallest fix = one line. Found 2026-07-16 on the first human attempt at C4; **D-48 (no DOM harness) is why it shipped**. Tracked: [`sprints/archive/2026-08_tier3_structural/followups/demo-officer-switcher-one-way-door.md`](sprints/archive/2026-08_tier3_structural/followups/demo-officer-switcher-one-way-door.md) · PROGRESS **D-65** |
