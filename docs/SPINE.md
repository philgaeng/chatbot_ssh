...
⚠ **`GRM-080` is the next free id.** (`GRM-067`–`GRM-069` were opened 2026-09-06 — the port measurement, the full-suite run it triggered, and the CI half of `GRM-068`'s fix; `GRM-070`–`GRM-072` by QA-04a and QA-04b; `GRM-073`–`GRM-075` by QA-04c, all three found by driving flows a reader of the code would have believed worked; `GRM-076` and `GRM-077` by QA-04d.)
