...
| 7 | Privacy & applicable laws | 🟠 **Partial — and nothing has happened yet** | The **assessment against the Individual Privacy Act 2018 and a 13-leg data-flow diagram** are written, each leg verified against code rather than inferred — which is how the **three storage-layer defects in §5** came to light. ⭐ **No genuine grievance has been processed on this platform**: every record is AI-generated seed data or a demo dummy, so every exposure is **prospective, not realised**, and the redaction work is a **go-live precondition rather than remediation**. Still missing: that redaction, and a lawyer's review (Q17) |
