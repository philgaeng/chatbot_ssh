...
| 7 | Privacy & applicable laws | 🟠 **Partial** | The assessment and the data-flow diagram exist and three storage-layer defects they found are fixed. **Outstanding:** unredacted egress to a third-party model provider, no deletion capability anywhere, and no legal review of the assessment (**Q15**). The breach procedure is now written — three decisions in it await a DOR owner |
...
| 9 | Do no harm by design | 🟢 **Mostly compliant** | Access control, audit log, SEAH isolation and anonymous intake are built, and content detection has two independent paths. **Outstanding:** a retention policy, third-party-PII redaction, and a measured SEAH detector. The breach procedure is written; three decisions in it await a DOR owner |
...
- **A breach procedure** — ✅ **written 2026-08-24**
  ([`../deployment/19_incident_response.md`](../deployment/19_incident_response.md)), and `SECURITY.md`
  now points at it. **Three decisions in it are blank** — who declares a breach, who is notified on what
  clock, and how a survivor is told when a SEAH case is exposed — because they belong to the implementing
  agency, not to engineering; the maintainer holds them on an interim basis and the document says so.
  Writing it surfaced a live defect: **Keycloak was recording no login events at all** (F-18), and the
  daily ops report was reporting `0` logins rather than "not recorded". Fixed the same day, forward-only.
...
1. A written **retention and deletion policy** — documentation, gated on the privacy assessment's legal
   review. The **breach procedure** is now written
   ([`../deployment/19_incident_response.md`](../deployment/19_incident_response.md)); what remains of it
   is three decisions for the Department of Roads, not engineering work.
