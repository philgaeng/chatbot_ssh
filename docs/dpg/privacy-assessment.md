...
| **L4** | Celery → **model provider** | ⚠ **Raw narrative, complainant name and phone, audio files** — unredacted | `LLM_services.py:64,114,153,250,353,420` | **Six call sites. Leaves Nepal on every classification.** ⏳ **Re-pointed 2026-08-18 (DPG-11):** the six sites now resolve their model and endpoint through `backend/config/llm_config.py` and build no client of their own. **What leaves the country is unchanged** — this is a routing change, not a privacy control; the destination is `LLM_BASE_URL`, which still defaults to `api.openai.com` (Q-10 flips it in Sprint 2). Redaction is Sprint 3 (DPG-33), and it hooks the factory rather than these six sites. See §4 |
| **L5** | Ticketing Celery → **model provider** | ⚠ Officer case notes verbatim; **the whole case timeline, including SEAH cases** | `ticketing/clients/llm_client.py:114,196,289` | **Three call sites.** Same destination, second independent client. A reviewer who redirects one and finds the other is entitled to distrust the rest of the submission. ⏳ **Re-pointed 2026-08-18 (DPG-12):** that reviewer's test now passes — both surfaces read one registry, and a test constructs both factories from one env change to prove it (T-17-d). **The content leaving the country is unchanged**; redaction is Sprint 3 |
