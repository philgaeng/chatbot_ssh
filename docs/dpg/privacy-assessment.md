...
> 3. ⭐ **Redaction is live on staging, and on nothing else.** Verified in the running image
>    2026-09-03: `pii_service.py` is deployed and the model-call chokepoint resolves `redact`. **The
>    email-boundary fixes are not** — they post-date that deploy. **DOR production has none of it**
>    and was not reachable to verify from here, so treat every control in this document as absent
>    there until someone checks.
...
3. ⚠ **Deployment, which is now a split answer rather than a flat no.** Staging runs the redaction
   layer, the log filter, the broker fix and the safeguarding escalate-only fix — verified in the
   running image. It does **not** run the email-boundary fixes, and Keycloak still records **0**
   authentication events there. **DOR production runs none of it and was not verified.** A control
   on a branch is evidence of intent; a control on one host of two is evidence about that host.
