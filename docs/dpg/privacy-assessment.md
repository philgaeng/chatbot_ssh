...
| **L4** | Celery → **model provider** | ⚠ **Raw narrative and audio files** — unredacted. ⚠ **Correction 2026-08-18: not the complainant's name or phone** — see below | `LLM_services.py:266,417,490` (+ ASR `:71`) | **Four reachable call sites, not six.** ⚠ **Corrected 2026-08-18 (D-35/D-37).** `extract_contact_info` and `extract_all_contact_info` — the two that would send contact details — **have no production caller**: nothing enqueues `extract_contact_info_task` outside a manual test script, and the second function has no reference at all (whole-repo grep). The live contact path is **deterministic** (`backend/actions/services/contact/phone.py::validate_complainant_phone`), with no model involved. **An inventory that overstates egress is not a safe error** — it spends the reader's trust on paths that do not exist. What does leave: the grievance narrative, which may contain contact details the complainant typed *inside it* — that is Sprint 3's subject, not this row's. See §4 |
