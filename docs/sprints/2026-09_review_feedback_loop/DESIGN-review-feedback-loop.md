...
|---|---|---|
| **D1** | Where the ticket lives | **GitHub Issues** — no schema, no migration, no fourth Alembic stream, and it is where the triage agent reads from (§2.3). ✅ **Resolved 2026-09-04.** This read *"verified: `philgaeng/chatbot_ssh` is a PUBLIC repository, so issues and any attached screenshot are world-readable — confirm, or pick a private tracker"*. [D-010](../../DECISIONS.md) picked: the repository is now **private**, so issues, screenshots and reviewer comments are not world-readable and this decision no longer rests on §2.4's synthetic-staging invariant alone. ⚠ §2.4 still matters — it is what lets a model read a screenshot at all — but it is no longer the *only* control. ⭐ Owned by [OM-07](../2026-09_operating_model/07-OM-07-intake-and-tracker.md), which unifies this vocabulary with the other four rather than adding a sixth. |
