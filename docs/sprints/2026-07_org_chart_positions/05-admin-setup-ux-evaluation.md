...
1. **Walk the flow on the running stack** (`docs/deployment/DOCKER.md`, bypass-auth mode) as each admin persona — `super_admin`, `country_admin` (standard), `country_admin` (seah), `project_admin`. Do a cold-start setup: empty-ish system → one org tree → officers → workflow → a launched project. Screenshot or note each step. Do **not** rely on reading code alone; experience the clicks.
