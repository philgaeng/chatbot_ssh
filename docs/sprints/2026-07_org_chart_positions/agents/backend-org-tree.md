...
4. Extend `locations.py`: new fields on GET/POST/PATCH, `tree=true` + `root_id` filter, cycle/unit_type/territory validation, tighten the gate to **org_admin standard / super_admin** via `admin_access`.
