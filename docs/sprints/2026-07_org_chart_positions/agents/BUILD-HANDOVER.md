...
- **Project participants & supervision (DECISION 2026-07-10 — [`DECISION-project-participants-and-supervision.md`](../DECISION-project-participants-and-supervision.md)):** the org project-**role** catalog is **removed** (`project_actor_roles` / `org_role` / `routing_org_role` gone). A project carries a single defaulted **`implementing_agency`** field (the signing ministry; routing/reporting anchor, gov/local-gov only) + optional **`project_donors`** (0..n). A donor's `donor-*` roles auto-populate the **last step's "Kept informed"** cast — **go-live blocks** if a donor isn't informed there (**SEAH-suppressed** on the SEAH track). **Supervisor** is a per-`(project, step)` reassignment authority (default = next step's Handler pool; **no org-tree derivation** — the `parent_organization_id` branch is deleted). **Go-live gates on staffing** (every level has an officer), not on org roles. **Affects OC-03/OC-04 + doc 13.**
