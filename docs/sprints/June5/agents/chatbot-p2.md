...
| ID        | Order | Summary                                                                                                                                |
| --------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **CB-01** | 4     | Record voice note button; submit without text if audio attached; 60–90s clips if no chunking; transcription via accessible API (async) |
| **CB-06** | 3     | Map pin location; fallback to existing ask-location form                                                                               |
| **CB-08** | 2     | Consent + EXIF location/time on image upload                                                                                           |
| **CB-09** | 1     | DUST category short path (pin → file → photos → optional contact)                                                                      |
