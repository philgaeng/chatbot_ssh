...
> **Status:** 🟡 **ROOT FIX LANDED 2026-07-16 — the one-way door is closed; C4 unblocked. Optional hardening (items 1/2/4/5) remains.** · **Owner:** portal (`channels/ticketing-ui`) · **Priority:** was high (blocked the sweep's most important section); now low (residual is defense-in-depth). Not a production security issue (bypass builds only).
>
> **What landed (DoD item 3 — the root fix):** `GET /users/roster` + `/users/roster/search` now use `require_admin_or_bypass` (`ticketing/api/dependencies.py`) instead of `require_admin` — **admin-only under Keycloak, any authenticated identity in dev bypass**. Switching to a non-admin officer now loads the roster, so the portal renders *as* that officer with a working switcher, and switching back works. **Verified live** (officer `GET /users/roster` 403→200 on the running container) and by `tests/ticketing/test_demo_switcher_roster.py` (bypass→200, keycloak→403); full `tests/ticketing` **607 passed / 5 skipped**. Backend-only — no `grm_ui` rebuild needed. **Items 1/2/4/5 below are now moot for the normal switch flow** (the catch branch and the hardcoded-super_admin display only fire on a roster *failure*, which no longer happens in bypass) but are kept as defense-in-depth.
> **Origin:** the sweep's §C4 ("Every tab at every role tier") instructs the tester to switch to `l1-officer@grm.local` and five other roles. Before the fix, **doing so locked you out of admin with no UI path back.** §C4 is the section the sweep itself calls out as *"if you only have time for one part of this sweep, do C4"* — so this defect blocked the highest-value manual check we have.
...
3. ✅ **DONE (2026-07-16) — Make the switcher not depend on admin.** Gated the roster on `require_admin_or_bypass`: `get_authenticated_user` when `bypass_enabled`, `require_admin` otherwise. **This is the root fix** — (1) only makes the trap recoverable, it does not stop you falling in. Chosen over "give the switcher a non-admin list source" because it is smaller, keeps a single roster endpoint, and preserves the Keycloak posture exactly (the roster is genuinely admin-only under real auth). Landed in `ticketing/api/dependencies.py` + the two routes in `ticketing/api/routers/users.py`; pinned by `tests/ticketing/test_demo_switcher_roster.py`.
...
5. **Regression test.** The portal has no DOM harness (D-48, tracked in [`settings-tab-render-tests.md`](settings-tab-render-tests.md)) — which is precisely why a switcher this broken shipped unnoticed. The **backend** side is now pinned (`tests/ticketing/test_demo_switcher_roster.py` — bypass grants, Keycloak denies). A **vitest on `AuthProvider`**'s roster-failure path (asserting the cookie is cleared) is still open, and only matters once item 1 is done.
