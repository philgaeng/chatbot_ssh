...
| 5 | Documentation | `docs/` (the full spec tree), `docs/deployment/DOCKER.md`, OpenAPI, `docs/_starter_kit/` | existing + **DPG-06** (the root `README.md` currently advertises a Rasa service that does not exist) |
| 6 | Data extraction | PostgreSQL, documented schema, non-proprietary export (XLSX reports, PDF closure docs) | existing |
| 7 | Privacy & applicable laws | Data-flow diagram; Individual Privacy Act 2018 assessment; PII redaction; encryption at rest and in transit | DPG-04, Sprint 3 |
| 8 | Standards & best practices | OpenAPI, OIDC (Keycloak), architectural invariants pinned by tests, GovStack alignment where applicable | existing + **DPG-05** (`SECURITY.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, templates — all verified absent) |
| 9 | Do no harm | Redaction of third-party PII; officer access controls; audit log | Sprint 3 + existing |
| 9a | Data privacy & security | Sprint 3 outputs; retention and deletion policy; breach procedure | DPG-04, Sprint 3 |
...
| 9 | **The LLM budget** | Sprint 2 is made almost entirely of inference calls, and there is no budget — voice transcription is already off for this reason. The CI job is the only *recurring* cost and the strongest indicator-4 artefact; cap it first, price DPG-23 before building it, and ask ADB for a metered line rather than a lump sum | ADB / agency | **Q-19** |
| 10 | **Sprint 3's person-name gap** | With DPG-32 deferred, deterministic redaction ships and person names go unredacted to a permanent third-party endpoint. Defensible as staging, **not** as a solved problem — an interim project-personnel deny-list is on the table | owner | in [`04` DPG-32](04-pii-redaction-spec.md#dpg-32) |
