...
| D-05 | ⚠ **Our own npm manifest declared no licence**, so `license-checker` reported `ticketing-ui@0.1.0` as `UNLICENSED` — while the repo root carried the Apache-2.0 `LICENSE` DPG-01 had just added. **A DPG reviewer running the same command would have found it first.** Fixed in both `package.json` files; the container still shows the old value until `grm_ui` is rebuilt | `channels/ticketing-ui/package.json`, `channels/REST_webchat/package.json` | DPG-02 ✅ fixed |
