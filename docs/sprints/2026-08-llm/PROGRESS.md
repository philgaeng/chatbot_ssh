...
| D-26 | 🔵 **Open — backlog** | DPG-01 / DPG-02 | ⚠ **CI has been red on `integration/stage` for its last 10 runs, 2026-08-08 → 2026-08-17 — so \"green in CI\" was never available to claim, and I claimed it anyway.** Pushing this branch produced the **identical failure shape**: `ui-checks` fails at **Lint** (2 `react-hooks/set-state-in-effect` errors out of 166 problems), which **skips the Build step**; `backend-tests` fails pytest (18 failed / 1143 passed) in project-type, back-fill and grievance-sync tests. ✅ `tests/repo` passes. **This sprint introduced no new failure** — verified step by step against the baseline run | **A permanently red build is indistinguishable from one nobody watches**, which is the root the sprint kept meeting from different angles: D-01 (27 files never collected), D-24 (branch prefix missing from triggers), and now this. ⚠ **It also blocks D-02b properly:** Build is skipped when Lint fails, and Build is the `next build` gate that deviation named for the `"use client"` question — so that gate **has never run in CI and cannot until the 2 lint errors are fixed**. The Docker build (D-02) remains the evidence, and its limits are now stated rather than glossed | ✅ [`followups/ci-has-been-red-for-ten-days.md`](followups/ci-has-been-red-for-ten-days.md) + `TODO.md` row |
