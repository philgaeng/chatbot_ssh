...
| P-02 | `gpt-5-nano` is the **live classification model**, not a stray reference | `LLM_services.py:232` | DPG-14.1 |
| P-03 | Classification builds a **shadow client** at `:201` that shadows the module-level one, then tests it with a guard that can never fire (`:202-203`) | `LLM_services.py:200-203` | DPG-14.2 |
| P-04 | ⚠ **Suspected** — ASR call passes `language_code=` where the SDK takes `language=`; if the SDK rejects it, transcription has never worked on this path. **Unverified: host has no `openai` (Docker-only)** | `LLM_services.py:48` | DPG-14.3 |
| P-05 | `extract_contact_info`'s `except` reads `response` at `:95`, which is unbound if the failure happened before `:78` → `UnboundLocalError` instead of the documented `{field_name: ""}` | `LLM_services.py:94-104` | DPG-10 / DPG-14 |
| P-06 | Two JSON-producing calls have **no `response_format` at all** — classification and translation ask for "strict JSON" in the prompt and absorb parse failures as empty results | `LLM_services.py:232`, `:324` | DPG-13 |
...
| P-09 | Translation error paths interpolate the whole `input_data` dict — including `grievance_description` — into a `ValueError` message that is then logged and may reach the Celery result backend | `LLM_services.py:337`, `:345` | DPG-34 |
| P-10 | `parse_llm_response` logs the **raw model response** on JSON parse error | `LLM_services.py:284` | DPG-34 |
| P-11 | `detect_sensitive_content_llm` **fails open** — LLM unreachable ⇒ `detected: False` on the SEAH path | `LLM_services.py:405-406` | DPG-15 / Q-14 |
...
| P-25 | Owner's Q-11 answer states translation and classification run **in one call**; the code has them as **two** (`:204-231` classify/summarise/follow-up; `:322` translate). The true and useful version: there is no translate-then-classify pipeline — the model consumes Nepali directly | `LLM_services.py:206-233`, `:324` | DPG-23, Q-11 |
