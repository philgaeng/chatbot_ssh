...
| ✅ **Q-19 — the LLM budget, answered 2026-08-20** | Raised by Q-13 (*"a few calls per day to nano, not transcription"*). **Sprint 2 is made of inference calls and nobody priced them.** Cap [DPG-24](03-open-models-spec.md#dpg-24) first — it is the only recurring cost — and price [DPG-23](03-open-models-spec.md#dpg-23) before building it · ✅ **A few hundred USD, owner-funded**, covering benchmarking **and** the pilot's own inference across two districts, with a costed proposal to the Nepal Government to follow. **Sprint 2 is unblocked.** ⚠ The envelope is *shared with production*, so DPG-23's sweep spends the pilot's runway — and DPG-24 is the only cost that outlives the demo months, which is why it is still the one to cap first |
