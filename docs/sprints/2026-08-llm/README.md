...
| "plus a `gpt-5-nano` reference" (implying a stray typo) | `gpt-5-nano` **is the live grievance-classification model** (`LLM_services.py:232`) — the primary AI path in the product | Not a cleanup item. It is the model selection that Sprint 2 has to benchmark against. |
