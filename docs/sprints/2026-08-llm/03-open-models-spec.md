...
| Classification runs on `gpt-3.5-turbo` | It runs on `gpt-5-nano` (`LLM_services.py:232`) | The "current" column of the benchmark must name the real baseline, or the delta is meaningless |
...
- **Sensitive-content detection is recall-first, not accuracy-first.** A missed harassment report is a
  ...
  encodes this (*"be extra sensitive as awareness around the issue is low"*, `LLM_services.py:374`).
