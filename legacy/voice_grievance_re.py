...
def submit_grievance():
    ...
    try:
        ...
        # Create grievance
        grievance_id = db_manager.grievance.create_or_update_grievance(data={'user_id': user_id, 
...
def process_single_audio_file(file_path: str, filename: str, language: str = 'en') -> Dict[str, Any]:
    ...
    # Create the base transcription task - exactly like in tests
    transcription_task = transcribe_audio_file_task.delay(
        file_path=file_path,
        language=language,
    ...
    if is_contact_info:
        # First get transcription, then process contact info
        contact_group = group(
            chain(
                extract_contact_info_task.s(emit_websocket=False),
        ...
        task_chain['tasks'] = contact_group.apply_async()
        task_chain['type'] = 'contact_info'
        
    elif is_grievance_details:
        # Chain: transcribe -> parallel(store transcription, classify) -> store
        classification_group = group(
            classify_and_summarize_grievance_task.s(emit_websocket=False),
            store_result_to_db_task.s()
        )
        task_chain['tasks'] = classification_group.apply_async()
        task_chain['type'] = 'grievance_details'
        
    else:
        # Just transcription and store
        task_chain['tasks'] = store_result_to_db_task.s().apply_async()
